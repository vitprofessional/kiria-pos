...
WHERE dip_readings.tank_id=fuel_tanks.id AND dip_readings.date_and_time < '$start_date'
    ...
    public function getpurchaseDetails($business_id, $location_id, $start_date, $end_date)
    {
        ...
        return $query->select(['contacts.name as cname', 'transactions.transaction_date', 'total_before_tax as amount'])->get();
    }
...
AND (pl.variation_id=variations.id)) as stock_price"),
    ...
    public function getCombinedStockSummaryReport(Request $request)
    {
        ...
        $stockReport = DB::table('products')
            ...
            ->whereDate('purchase_lines.updated_at', '>=', $start_date)
            ->whereDate('purchase_lines.updated_at', '<=', $end_date)
            ->groupBy(
                'products.id',
                'products.sku',
                'products.name',
    ...
    public function stockPurchaseSaleReport(Request $request)
    {
        ...
        $stockReport = DB::table('products')
            ...
            ->where(function ($query) use ($start_date, $end_date) {
                $query->whereDate('purchase_lines.updated_at', '>=', $start_date)
                    ->whereDate('purchase_lines.updated_at', '<=', $end_date)
                    ->orWhere(function ($query) use ($start_date, $end_date) {
                        $query->whereDate('transaction_sell_lines.updated_at', '>=', $start_date)
                            ->whereDate('transaction_sell_lines.updated_at', '<=', $end_date);
                    });
            })
            ->select(
                'products.sku AS sku',
    ...
    public function getCustomerOutstandingReport(Request $request)
    {
        ...
        $outstandingReport = DB::table('contacts')
            ...
            ->whereDate('contact_ledgers.updated_at', '>=', $start_date)
            ->whereDate('contact_ledgers.updated_at', '<=', $end_date)
