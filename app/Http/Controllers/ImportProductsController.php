...
class ImportProductsController extends Controller
{
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (!empty($sub_category_name)) {
                        if(empty( $product_array['category_id'])){
                            $is_valid = false;
                            $error_msg = "Category name of sub-category $sub_category_name should no be empty";
                            break;
                        }
                        $sub_category = Category::firstOrCreate(
                            ['business_id' => $business_id, 'name' => $sub_category_name],
                            ['created_by' => $user_id, 'parent_id' => $product_array['category_id']]
                        );
                    ...
                    $product_array['vat_claimed'] = strtolower(trim($value[37])) == 'yes' ? 1: 0;
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        if (!empty($value[20]) && $enable_stock == 1) {
                            $dateValue = trim($value[36]);
                            // Check if value is numeric (Excel date format)
                            if (is_numeric($dateValue)) {
                                // $transaction_date_temp = Carbon::instance(Date::excelToDateTimeObject($dateValue));
                                $transaction_date_temp = \Carbon::createFromFormat('Y-m-d', \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($dateValue)->format('Y-m-d'));
                            } else {
                                $transaction_date_temp = \Carbon::createFromFormat('d/m/Y', $dateValue)->format('Y-m-d');
                            }
                ...
                if (!empty($formated_data)) {
                    foreach ($formated_data as $index => $product_data) {
                        ...
                        if (isset($product_data['opening_stock_details'])) {
                            $availableQTY = $product_data['opening_stock_details']['quantity'];
                            unset($product_data['opening_stock_details']);
                        }
                        
                        $product_data['date'] = $transaction_date;
                        ...
                        if ($product->type == 'single') {
                         ...
                         // Assigned Store
                        $location = BusinessLocation::where('business_id', $business_id)->first();
                        $store = Store::where('business_id', $business_id)->where('location_id', $location->id)->first();
                        $productVariation = ProductVariation::where('product_id', $product->id)->first();
                        $variation = Variation::where('product_variation_id', $productVariation->id)->where('product_id', $product->id)->first();
                        $variationStoreDetail = Variation_store_detail::where('product_id', $product->id)->first();
                        DB::insert('insert into product_locations (product_id, location_id) values (?, ?)', array($product->id, $location->id));
                        if($variationStoreDetail){
                            $variationStoreDetail->update([
                        ...
                        // \Log::debug("product->enable_stock $product->enable_stock", ['os_transaction'=>$os_transaction]);
                        if ($product->enable_stock) {
                            if (!empty($product->stock_type)) {
                                ...
                                $this->transactionUtil->createAccountTransaction($os_transaction, 'credit', $opening_balance_equity_id, $os_transaction->final_total);
                            }
                        }
                    }
                }
    ...
    private function addOpeningStock($opening_stock, $product, $business_id, $transaction_date)
    {
        ...
        $this->productUtil->updateProductQuantityStore($opening_stock['location_id'], $product->id, $variation->id, $opening_stock['quantity']);
        ...
        if (isset($variations['variations'][0]['opening_stock'])) {
            ...
            foreach ($variations['variations'] as $variation_os) {
                if (!empty($variation_os['opening_stock'])) {
                    ...
                    $this->productUtil->updateProductQuantityStore($location_id, $product->id, $variation->id, $opening_stock['quantity']);
                }
