    ...
    */
        ...
        try {
            $validator = $request->validate(
                ...
                [
                    ...
                    'district.required' => __('validation.required', ['attribute' => __('business.district')]),
            ...
            $business_location = $request->only(['name', 'country', 'district','state', 'city', 'zip_code', 'landmark', 'website', 'mobile', 'alternate_number']);
            //Create the business
    ...
    public function postVisitorRegister(Request $request)
    {
        ...
        try {
            ...
            $business_location = $request->only(['name', 'country', 'state','district', 'city', 'zip_code', 'landmark', 'website', 'mobile', 'alternate_number']);
            //Create the business
            $business_details['owner_id'] = $user->id;
            if (!empty($business_details['start_date'])) {
                $business_details['start_date'] = \Carbon::createFromFormat(config('constants.default_date_format'), $business_details['start_date'])->toDateString();
    ...
    public function postPatientRegister(Request $request)
    {
        ...
        try {
            ...
            $data_details = array(
                ...
                'marital_status'  => (int)$request->marital_status,
                'blood_group'  => (int)$request->blood_group,
            ...
            /*
            $business_location = $request->only(['address', 'country', 'state','district', 'city', 'mobile']);
            ...
            */
            ...
            if( $location_resp['success'] ){
                $business_location = $location_resp['data'];
                $business_location['name'] = $business_location['address'];
                $business_location['location_access_type'] = 'registration';
                $business_location['mobile'] = $request->mobile;
                $new_location = $this->businessUtil->addLocation($business->id, $business_location, 1, 1);
                
                LocationUtil::storeUserLocation( $user->id, 'business_registration', $location_resp['data'] );
            }
            $this->businessUtil->newBusinessDefaultResources($business->id, $user->id);
            $package_id = $request->get('package_id', null);
            $this->businessUtil->addReferral($request->referral_code, 'patient', $business->id, $package_id);
    ...
    public function getBusinessSettings()
    {
        ...
        return view('business.settings', compact('help_explanations','weighing_scale_setting','payment_types', 'get_permissions', 'enabled_moudle_by_subscription', 'business_categories', 'enable_duplicate_invoice', 'stores', 'search_product_settings', 'package_permissions', 'business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings','businessLocations'));
    }
    /**
    ...
    {
        Log::info($request->contact_fields);
        if (!auth()->user()->can('business_settings.access')) {
            abort(403, 'Unauthorized action.');
        }
        try {
            ...
            $business_details = $request->only([
                ...
                'rp_expiry_type', 'custom_labels', 'currency_precision', 'quantity_precision', 'reg_no', 'contact_fields','service_addition_method','font_size','font_style'
            ]);
            ...
            // NGUYEN DUC VIET Comment:
            // Because this input which have name is "on_product_expiry" does not exist in business settings form.
            // So we need to check exist first.
            if (isset($business_details['on_product_expiry']) && $business_details['on_product_expiry'] == 'keep_selling') {
//            if ($business_details['on_product_expiry'] == 'keep_selling') {
           ...
           $dataToSave = [
    'vat_no' => $request->contact_fields['vat_no'] ?? null,
    'credit_limit' => $request->contact_fields['customer_credit_limit'] ?? null,
    'mobile' => $request->contact_fields['customer_mobile'] ?? null,
    'address' => $request->contact_fields['customer_address'] ?? null,
    'state' => $request->contact_fields['customer_state'] ?? null,
    'tax_number' => $request->contact_fields['customer_tax_number'] ?? null,
    'confirm_password' => $request->contact_fields['customer_confirm_password'] ?? null,
    'sub_customer' => $request->contact_fields['sub_customer'] ?? null,
    'passport_nic_no' => $request->contact_fields['passport_nic_no'] ?? null,
    'need_to_send_sms' => $request->contact_fields['need_to_send_sms'] ?? null,
    'landline' => $request->contact_fields['customer_landline'] ?? null,
    'address_line_2' => $request->contact_fields['address_line_2'] ?? null,
    'country' => $request->contact_fields['customer_country'] ?? null,
    'pay_term' => $request->contact_fields['customer_pay_term'] ?? null,
    'email' => $request->contact_fields['customer_email'] ?? null,
    'vehicle_no' => $request->contact_fields['vehicle_no'] ?? null,
    'passport_nic_image' => $request->contact_fields['passport_nic_image'] ?? null,
    'credit_notification_type' => $request->contact_fields['credit_notification_type'] ?? null,
    'alternate_contact_number' => $request->contact_fields['customer_alternate_contact_number'] ?? null,
    'customer_group' => $request->contact_fields['customer_customer_group'] ?? null,
    'add_more_mobile_numbers' => $request->contact_fields['add_more_mobile'] ?? null,
    'city' => $request->contact_fields['customer_city'] ?? null,
    'landmark' => $request->contact_fields['customer_landmark'] ?? null,
    'password' => $request->contact_fields['customer_password'] ?? null,
    'address_line_3' => $request->contact_fields['address_line_3'] ?? null,
    'signature' => $request->contact_fields['signature'] ?? null,
    'updated_at' => now(),
];
        ...
        if ($airlineCustomer) {
            $airlineCustomer->update($dataToSave);
        } else {
            // Create a new record
            $airlineCustomer = AirlineFormSettingCustomer::create(array_merge($dataToSave, [
                'business_id' => $business_id,
            ...
            $checkboxes = [
                ...
                'enable_lot_number', 'enable_racks', 'enable_row', 'enable_position', 'show_avai_qty_in_qr_catalogue', 'show_in_catalogue_page', 'enable_sub_units', 'enable_line_discount','duplicate_orders_allowed'
            ];
    ...
    public function testSmsConfiguration(Request $request)
    {
        try {
            ...
            if (!empty($sms_settings['test_number'])) {
                $response = $this->businessUtil->sendSms($data,'Test');
            } else {
