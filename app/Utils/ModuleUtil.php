...
class ModuleUtil extends Util
{
    ...
    {
        // $cache_duration = 3000; // 50 Minutes
        
        // $is_available = Cache::remember("is_available_Superadmin", $cache_duration, function () {
        //     return Module::has('Superadmin');
        // });
        $is_available = Module::has('Superadmin');
        ...
        if ($is_available) {
            // return Cache::remember("package_{$business_id}_{$permission}_{$callback_function}", $cache_duration, function () use ($business_id, $permission, $callback_function) {
                $package = \Modules\Superadmin\Entities\Subscription::current_subscription($business_id);
