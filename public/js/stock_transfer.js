$(document).ready(function() {
        ...
        $('#search_product_for_srock_adjustment')
            .autocomplete({
                source: function(request, response) { 
                    ...
                    $.getJSON(
                        '/products/list-sa',
                        { location_id: $('#location_id').val(),store_id : $("#from_store").val(), term: request.term, module: $("#module").val() },
                        response
                    );
                },
                minLength: 1,
                response: function(event, ui) {
                    ...
                    if (ui.content.length == 1) {
                        ...
                        if (ui.item.store_qty > 0 && ui.item.enable_stock == 1) {
                            $(this)
                                .data('ui-autocomplete')
                                ._trigger('select', 'autocompleteselect', ui);
                            $(this).autocomplete('close');
                        }
                    } else if (ui.content.length == 0 && search_term.length > 2) {
                        swal(LANG.no_products_found);
                    }
                },
                focus: function(event, ui) {
                    if (ui.item.store_qty <= 0) {
                        return false;
                    }
                },
    ...
    // COde start from here
     ...
     All_Stock_Transfer = $('#All_Stock_Transfer').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/stock-transfers',
        columnDefs: [
        ...
        columns: [
            ...
            { data: 'Type', name: 'Type' },
            { data: 'Quantity_Issued', name: 'Quantity_Issued' },
...
function stock_transfer_product_row(variation_id) {
    ...
    $.ajax({
        method: 'POST',
        url: '/stock-adjustments-new/get_product_row_stock_transfer',
        data: { row_index: row_index, variation_id: variation_id, location_id: location_id, store_id: $("#from_store").val() },
        dataType: 'html',
        success: function(result) {
