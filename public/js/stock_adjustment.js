        ...
        $('#search_product_for_srock_adjustment')
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        '/products/list-sa',
                        { location_id: $('#location_id').val(), term: request.term, module: $("#module").val() },
                        response
                    );
                },
                minLength: 2,
...
function update_table_total() {
    ...
    var stock_adjustment_type = $('#stock_adjustment_type').val();
    if(table_total > 0){
        if(stock_adjustment_type == 'increase'){
            // remain positive
        } else if(stock_adjustment_type == 'decrease') {
            table_total = table_total * -1; // show negative
        } else {
