$(document).ready(function () {
    ...
    var defaultDate = moment($('input[name="transaction_date"]').val(), moment_date_format + ' ' + moment_time_format);

    // $('#transaction_date').daterangepicker({
    //     timePicker: true,
    //     singleDatePicker: true,
    //     showDropdowns: true,
    //     minYear: 1901,total_after_tax
    //     maxYear: parseInt(moment().format('YYYY'), 10),
    //     startDate: defaultDate,  // Set the default date and time
    //     timePicker24Hour: true,  // Use 24-hour format
    //     locale: {
    //         format: moment_date_format + ' ' + moment_time_format  // 24-hour format for displaying date and time
    //     }
    // });
    ...
    $(document).on('input', '.purchase_quantity', function () {
        ...
        row.find('.row_subtotal_before_tax').text(
            __currency_trans_from_en(sub_total_before_tax, false, true, 6)
        );
        __write_number(
        ...
        row.find('.row_subtotal_after_tax').text(
            __currency_trans_from_en(sub_total_after_tax, false, true, 6)
        );
        __write_number(row.find('input.row_subtotal_after_tax_hidden'), sub_total_after_tax, true, 6);
    ...
    $(document).on('input', '.purchase_unit_cost_without_discount', function () {
        ...
        __write_number(row.find('input.purchase_unit_cost'), purchase_before_tax, true, 6);
        ...
        __write_number(row.find('input.purchase_unit_cost_after_tax'), purchase_after_tax, true, 6);
        row.find('.row_subtotal_after_tax').text(
            __currency_trans_from_en(sub_total_after_tax, false, true, 6)
        );
        __write_number(row.find('input.row_subtotal_after_tax_hidden'), sub_total_after_tax, true, 6);
        ...
        row.find('.purchase_product_unit_tax_text').text(
            __currency_trans_from_en(tax, false, true, 6)
        );
        __write_number(row.find('input.purchase_product_unit_tax'), tax, true, 6);
    ...
    $(document).on('input', '.purchase_unit_cost', function () {
        var row = $(this).closest('tr');
        var quantity = __read_number(row.find('input.purchase_quantity'), true);
    ...
    $(document).on('input', '.purchase_unit_cost_after_tax', function () {
        var row = $(this).closest('tr');
        ...
        __write_number(row.find('.purchase_unit_cost'), purchase_before_tax, true, 6);
        ...
        row.find('.purchase_product_unit_tax_text').text(__currency_trans_from_en(tax, true, true, 6));
        __write_number(row.find('input.purchase_product_unit_tax'), tax, true, 6);
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        columns: [
            ...
            { data: 'total_after_tax', name: 'total_after_tax' },
            { data: 'final_total', name: 'final_total' },
            { data: 'payment_due', name: 'payment_due', orderable: false, searchable: false },
            // { data: 'payment_method', name: 'payment_method', orderable: false, searchable: false, visible: false },
            { data: 'lot_number', name: 'pl.lot_number' },
            { data: 'added_by', name: 'u.first_name' },
        ],
        fnDrawCallback: function (oSettings) {
            var total_purchase = sum_table_col($('#purchase_table'), 'final_total1');
    ...
    vat_purchase_table = $('#vat_purchase_table').DataTable({
        ...
        fnDrawCallback: function (oSettings) {
            var total_purchase = sum_table_col($('#vat_purchase_table'), 'final_total');
            $('#footer_purchase_total').text(total_purchase);

            var total_due = sum_table_col($('#vat_purchase_table'), 'payment_due');
            $('#footer_total_due').text(total_due);

            var total_purchase_return_due = sum_table_col($('#vat_purchase_table'), 'purchase_return');
            $('#footer_total_purchase_return_due').text(total_purchase_return_due);

            $('#footer_status_count').html(__sum_status_html($('#vat_purchase_table'), 'status-label'));
    ...
    $('table#purchase_entry_table').on('change', 'select.sub_unit', function () {
        ...
        __write_number(sp_element, unit_sp, true, 6);

        var cp_element = tr.find('input.purchase_unit_cost_without_discount');
        __write_number(cp_element, unit_cost, true, 6);
        cp_element.change();
...
function unloadTankQtyMatch() {
    ...
    $('.check_tank_row').each(function (index) {
        ...
        if (stock_match == '1') {
            ...
            if($('#add_purchase_form #status').length  && $('#add_purchase_form #status').val() != 'received'){
...
function update_purchase_entry_row_values(row) {
    if (typeof row != 'undefined') {
        ...
        row.find('.row_subtotal_before_tax').text(
            __currency_trans_from_en(row_subtotal_before_tax, false, true, 6)
        );
        __write_number(row.find('.row_subtotal_before_tax_hidden'), row_subtotal_before_tax, true, 6);
        __write_number(row.find('.purchase_product_unit_tax'), unit_product_tax, true, 6);
        row.find('.purchase_product_unit_tax_text').text(
            __currency_trans_from_en(unit_product_tax, false, true, 6)
        );
        row.find('.purchase_unit_cost_after_tax').text(
            __currency_trans_from_en(unit_cost_price_after_tax, true, 6)
        );
        row.find('.row_subtotal_after_tax').text(
            __currency_trans_from_en(row_subtotal_after_tax, false, true, 6)
        );
        __write_number(row.find('.row_subtotal_after_tax_hidden'), row_subtotal_after_tax, true, 6);
...
function update_row_price_for_exchange_rate(row) {
    var exchange_rate = $('input#exchange_rate').val() ?? 0;
    ...
    __write_number(row.find('.purchase_unit_cost'), purchase_unit_cost, true, 6);

    var row_subtotal_before_tax_hidden =
        __read_number(row.find('.row_subtotal_before_tax_hidden'), true) / exchange_rate;
    row.find('.row_subtotal_before_tax').text(
        __currency_trans_from_en(row_subtotal_before_tax_hidden, false, true, 6)
    );
    __write_number(
        row.find('input.row_subtotal_before_tax_hidden'),
    ...
    __write_number(row.find('input.purchase_product_unit_tax'), purchase_product_unit_tax, true, 6);
    row.find('.purchase_product_unit_tax_text').text(
        __currency_trans_from_en(purchase_product_unit_tax, false, true, 6)
    );
    ...
    row.find('.row_subtotal_after_tax').text(
        __currency_trans_from_en(row_subtotal_after_tax_hidden, false, true, 6)
    );
}
...
function iraqi_dinnar_selling_price_adjustment(row) {
    ...
    __write_number(row.find('input.default_sell_price'), default_sell_price, true, 6);
...
function update_table_total() {
    ...
    $('#total_st_before_tax').text(__currency_trans_from_en(total_st_before_tax, true, true, 6));
    __write_number($('input#st_before_tax_input'), total_st_before_tax, true, 6);

    $('#total_subtotal').text(__currency_trans_from_en(total_subtotal, true, true, 6));
    __write_number($('input#total_subtotal_input'), total_subtotal, true, 6);
    __write_number($('input#final_total_input'), total_subtotal, true, 6);

    $('#total_payable').text(__currency_trans_from_en(total_subtotal, true, true, 6));
    $('.price_total').text(__currency_trans_from_en(total_subtotal, true, true, 6));
    __write_number($('input#total_subtotal_input'), total_subtotal, true, 6);
}
...
function update_grand_total() {
    ...
    $('#purchase_entry_table tbody')
        ...
        .each(function () {
            ...
            total_tax += (__read_number($(this).find('.purchase_product_unit_tax'), true)) * (__read_number($(this).find('.purchase_quantity'), true));
            
            total_discount += ((__read_number($(this).find('.inline_discounts'), true) / 100) * (__read_number($(this).find('.purchase_unit_cost_without_discount'), true))) * (__read_number($(this).find('.purchase_quantity'), true));
            
        });


    var discount = total_discount;
    $('#discount_calculated_amount').text(__currency_trans_from_en(discount, true, false, 6));
    __write_number($("#discount_amount"),discount, true, 6);

    var tax = total_tax;
    __write_number($('input#tax_amount'), tax, true, 6);
    $('#tax_calculated_amount').text(__currency_trans_from_en(tax, true, false, 6));

    //Calculate shipping
    var shipping_charges = __read_number($('input#shipping_charges'), true);

    var price_adjustment = __read_number($('input#price_adjustment'), true);

    //Calculate Final total
    grand_total = total_subtotal + shipping_charges + price_adjustment;
    ...
    $('#grand_total').text(__currency_trans_from_en(grand_total, true, false));

    $('#payment_due').text(__currency_trans_from_en(due, true, false, 6));
...
$('button#add-payment-row').click(function () {
    ...
    // console.log('Add Payment Clicked', $('#cash_account_id').val(), amountToAdd); 
    $.ajax({
        method: 'GET',
...
function calculate_billing_details(price_total) {
    ...
    var total_payable = price_total + order_tax - discount + shipping_charges + price_adjustment + packing_charge;
...
function reset_pos_form() {
    ...
    __write_number($('input#price_adjustment'), $('input#price_adjustment').data('default'));
