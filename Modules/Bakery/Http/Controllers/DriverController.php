...
class DriverController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $drivers = BakeryDriver::leftjoin('users', 'bakery_drivers.created_by', 'users.id')
                ->where('bakery_drivers.business_id', $business_id)
                ->select([
                    'bakery_drivers.*',
                ...
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if (auth()->user()->can('fleet.drivers.edit')) {
                            $html .= '<li><a href="#" data-href="' . action('\Modules\Bakery\Http\Controllers\DriverController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        }
                        if (auth()->user()->can('fleet.drivers.edit')) {
                            $html .= '<li><a href="#" data-href="' . action('\Modules\Bakery\Http\Controllers\DriverController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</a></li>';
                        }
    ...
    public function create()
    {
        ...
        return view('bakery::drivers.create')->with(compact(
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $driver = BakeryDriver::create($data);
    ...
    {
        $business_id = request()->session()->get('business.id');
        $driver_dropdown = BakeryDriver::where('business_id', $business_id)->pluck('driver_name', 'id');
        $view_type = request()->tab;
        $driver = BakeryDriver::find($id);
        $contact_id = $id;

        return view('bakery::drivers.show')->with(compact(
            'driver_dropdown',
            'view_type',
            'driver',
            'contact_id'
    ...
    public function edit($id)
    {
        ...
        return view('bakery::drivers.edit')->with(compact(
            'driver'
        ));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            BakeryDriver::where('id', $id)->update($data);
    ...
    {
        try {
            BakeryDriver::where('id', $id)->delete();
