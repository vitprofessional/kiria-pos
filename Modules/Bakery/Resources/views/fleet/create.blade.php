<div class="modal-dialog modal-xl" role="document" style="height: 95vh; overflow-y: auto">
  <div class="modal-content">
    ...
    {!! Form::open(['url' =>
    action('\Modules\Bakery\Http\Controllers\FleetController@store'), 'method' =>
          ...
          <div class="d-flex" style="justify-content: space-between">
                ...
                <div class="d-flex" style="justify-content: end;">
                    <button type="button" class="btn btn-default bg-white btn-flat btn-modal" style="margin-right: 20px"
                            data-href="{{action('\Modules\Bakery\Http\Controllers\DriverController@create')}}" data-container=".contact_modal">
                        @lang( 'bakery::lang.add_new_driver' )
                    </button>
                    <button
                      type="button"
                      class="close"
      ...
      <div class="modal-body">
        ...
        <div class="col-md-3">
          <div class="input-group">
            {!! Form::label('contact_id', __( 'fleet::lang.driver' )) !!} {!!
            Form::select('contact_id[]', $drivers, null, ['class' =>
            'form-control drivers_dropdown select2 first_customer', 'required', 'placeholder' => __(
        ...
        <div class="col-md-2">
          <div class="form-group">
            {!! Form::label('invoice_no', __( 'bakery::lang.tour_no'
            )) !!} {!! Form::text('invoice_no[]', null, ['class' =>
            'form-control first_invoice', 'placeholder' => __( 'bakery::lang.tour_no' ),
            'id' => 'invoice_no']); !!}
          </div>
        </div>
...
<script>
  ...
  $(document).off('submit', "#driver_add_form").on('submit', "#driver_add_form", function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.success){
                ...
                $(".drivers_dropdown").append('<option value="'+response.data.id+'">'+response.data.driver_name+'</option>').val(response.data.id).trigger('change');
  ...
  $(document).off("click", "#add_opening_balance").on("click", "#add_opening_balance", function () {
    var html =
      ...
      ` {!!
            Form::select('contact_id[]', $drivers, null, ['class' =>
            'form-control drivers_dropdown select2_` +
      index +
      ...
      `
                    ...
                    <div class="col-md-2">
                       ...
                       ` +
      ` {!! Form::text('invoice_no[]', null, ['class' =>
            'form-control', 'placeholder' => __( 'bakery::lang.tour_no' ),
            'id' => 'invoice_no']); !!}` +
      `
                        </div>
                    </div>
  ...
  $("input[name=vehicle_number]").change(function () {
    ...
    $.ajax({
      type: "POST",
      url: "{{ route('bakery_vehicle_check') }}",
      data: {
              ...
              if(i== 0){
                  ...
                  `<div class="adddiv"><div class="col-md-3">
                                    <div class="form-group">
                                        ` +
                  ` {!!
                        Form::select('contact_id[]', $drivers, `+ allcontacts[i]+`, ['class' =>
                  ...
                  ` {!! Form::text('invoice_no[]', `+ allinvoices[i]+`, ['class' =>
                        'form-control', 'placeholder' => __( 'fleet::lang.tour_no' ),
                        'id' => 'invoice_no']); !!}` +
                  `
                                    </div>
                                </div>
