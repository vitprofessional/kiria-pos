...
<section class="content-header">
        ...
        <div class="col-md-12 dip_tab">
            <div class="settlement_tabs">
                <ul class="nav nav-tabs">
                    @if(!empty($pacakge_details['bakery_drivers']))
                    <li class=" @if(session('status.tab') == 'drivers' || empty(session('status.tab'))) active @endif">
                    ...
                    @if(!empty($pacakge_details['bakery_vehicles']))
                    <li class=" @if(session('status.tab') == 'vehicles') active @endif">
                        <a style="font-size:13px;" href="#vehicles" data-toggle="tab">
                            <i class="fa-solid fa-car"></i><strong>Vehicles</strong>
                        </a>
                    </li>
                    @endif
                    
                    @if(!empty($pacakge_details['bakery_starting_no']))
                    <li class=" @if(session('status.tab') == 'route_invoice_number') active @endif">
                        <a style="font-size:13px;" href="#route_invoice_number" data-toggle="tab">
                            # <strong>Starting No</strong>
                        </a>
                    </li>
                    @endif
                    
                    @if(!empty($pacakge_details['bakery_list_products']))
                    <li class=" @if(session('status.tab') == 'route_product') active @endif">
                        <a style="font-size:13px;" href="#route_product" data-toggle="tab">
                            <i class="fa fa-cubes"></i> <strong>List Product</strong>
                        </a>
                    </li>
                    @endif

                    <li class=" @if(session('status.tab') == 'routes') active @endif">
                        <a style="font-size:13px;" href="#routes" data-toggle="tab">
                            <i class="fa fa-cubes"></i> <strong>Routes</strong>
                        </a>
                    </li>
                    
                    <li class=" @if(session('status.tab') == 'users') active @endif">
                        <a style="font-size:13px;" href="#users" data-toggle="tab">
                             <strong>@lang('bakery::lang.users')</strong>
                        </a>
                    </li>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane @if(session('status.tab') == 'drivers' || empty(session('status.tab'))) active @endif" id="drivers">
             @include('bakery::drivers.index')
        </div>
        
        <div class="tab-pane  @if(session('status.tab') == 'vehicles') active @endif" id="vehicles">
            @include('bakery::fleet.index')
        </div>
        
        <div class="tab-pane  @if(session('status.tab') == 'route_invoice_number') active @endif" id="route_invoice_number">
            @include('bakery::bakery_invoice_number.index')
        </div>
        
         <div class="tab-pane  @if(session('status.tab') == 'route_product') active @endif" id="route_product">
           @include('bakery::bakery_product.index')
        </div>


        <div class="tab-pane  @if(session('status.tab') == 'routes') active @endif" id="routes">
           @include('bakery::routes.index')
        </div>
        
        <div class="tab-pane  @if(session('status.tab') == 'users') active @endif" id="users">
            @include('bakery::bakery_users.partials.list_users')
        </div>
...
$(document).ready( function(){
    var columns = [
            ...
            { data: 'sold_amount', name: 'sold_amount',searchable: false },
            
            { data: 'commission_type', name: 'commission_type' },
            { data: 'commission_rate', name: 'commission_ap' },
            
            { data: 'commission_amount', searchable: false  },
            { data: 'current_balance', name: 'current_balance', searchable: false }
        ];

    list_bakery_users_table = $('#list_bakery_users_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: {
            url: '{{action('\Modules\Bakery\Http\Controllers\BakeryUserController@index')}}',
            data: function(d) {
                d.location_id = $('select#bakery_location_id').val();
                d.pump_operator = $('select#bakery_pump_operator').val();
                d.status = $('select#bakery_status').val();
            },
        },
        
        columns: columns,
        fnDrawCallback: function(oSettings) {
            // var sold_fuel_qty = sum_table_col($('#list_bakery_users_table'), 'sold_fuel_qty');
            // $('#footer_sold_fuel_qty').text(sold_fuel_qty);
            // var sale_amount_fuel = sum_table_col($('#list_bakery_users_table'), 'sale_amount_fuel');
            // $('#footer_sale_amount_fuel').text(sale_amount_fuel);
            // var commission_amount = sum_table_col($('#list_bakery_users_table'), 'commission_amount');
            // $('#footer_commission_amount').text(commission_amount);
            // var excess_amount = sum_table_col($('#list_bakery_users_table'), 'excess_amount');
            // $('#footer_excess_amount').text(excess_amount);
            // var short_amount = sum_table_col($('#list_bakery_users_table'), 'short_amount');
            // $('#footer_short_amount').text(short_amount);

            __currency_convert_recursively($('#list_bakery_users_table'));
        },
    });

    $('#bakery_location_id, #bakery_pump_operator, #bakery_status').change(function(){
        list_bakery_users_table.ajax.reload();
    });
