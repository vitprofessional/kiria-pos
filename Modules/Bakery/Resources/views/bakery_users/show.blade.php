@extends('layouts.app')
@section('title', __('bakery::lang.bakery_user'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header no-print">
    <h1>{{ __('bakery::lang.bakery_user') }}</h1>
</section>
...
<section class="content no-print">
    <div class="hide print_table_part">
        ...
        <div style="width: 100%;">
            <div class="info_col">
                @include('bakery::bakery_users.contact_basic_info')
            </div>
        </div>
    </div>
    <div class="row">
            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content" style="background: #fbfcfc;">
                    <div class="tab-pane
                        ...
                        @include('bakery::bakery_users.partials.contact_info_tab')
                    </div>
                    <div class="tab-pane
                                @if(!empty($view_type) &&  $view_type == 'ledger')
                                    active
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                @include('bakery::bakery_users.partials.ledger_tab')
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                @include('bakery::bakery_users.partials.documents_and_notes_tab')
                                @endcomponent
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $.ajax({
        method: "GET",
        dataType: "html",
        url: "{{action('\Modules\Bakery\Http\Controllers\BakeryUserDocumentAndNoteController@getDocAndNoteIndexPage')}}",
        async: false,
...
function initializeDocumentAndNoteDataTable() {
    documents_and_notes_data_table = $('#documents_and_notes_table').DataTable({
            ...
            serverSide: true,
            ajax:{
                url: "{{action('\Modules\Bakery\Http\Controllers\BakeryUserDocumentAndNoteController@index')}}",
                data: function(d) {
...
<script>
    ...
    $('#pump_operator_id').change( function() {
        if ($(this).val()) {
            window.location = "{{url('/bakery/bakery-users')}}/" + $(this).val();
        }
    });
</script>
@endsection