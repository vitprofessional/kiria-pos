<div class="modal-dialog" role="document" style="width: 55%">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <input type="hidden" name="invoice_no" value="{{$property_sell->invoice_no}}">
            <input type="hidden" name="transaction_date" value="{{$property_sell->transaction_date}}">
            <input type="hidden" name="block_sold_price" value="{{$property_sell->block_sold_price}}">
            ...
            <div class="col-md-12">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input placeholder="@lang('property::lang.block_sold_price')" required="" class="form-control"
                               readonly type="text" value="{{@num_format($property_sell->block_sold_price)}}">
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        {!! Form::label('other_payment', __( 'property::lang.other_charges' ) . ':*') !!}
                        {!! Form::text('other_payment', null, ['placeholder' => __( 'property::lang.other_charges' ),
                        'required', 'class' => 'form-control']); !!}
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        {{-- ABHI ADD DEFAULT VALUE TOTAL PAYABLE  --}}
                        {!! Form::label('total_payable', __( 'property::lang.total_payable' ) . ':*') !!}
                        {!! Form::text('total_payable', (int)$property_sell->block_sold_price, ['placeholder' => __( 'property::lang.total_payable'
                        ),
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        {!! Form::label('reserved_amount', __( 'property::lang.reserved_amount' ) . ':*') !!}
                        {!! Form::text('reserved_amount', null, ['placeholder' => __( 'property::lang.reserved_amount'
                        ),
                        'required', 'class' => 'form-control', 'readonly'=>'ture']); !!}
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        'required', 'class' => 'form-control', 'readonly' => true]); !!}
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        event.charCode >= 48 && event.charCode <= 57']); !!} </div>
                    <div class="clearfix">
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        {!! Form::label('last_installment_date', __( 'property::lang.last_installment_date' ) .
                        ':*')
                        !!}
                        {!! Form::text('last_installment_date', null, ['placeholder' => __(
                        'property::lang.last_installment_date' ),
                        'required', 'class' => 'form-control', 'readonly' => 'true']); !!}
                    </div>
                </div>
...
*/
    // ABHI SCRIPT
    var total_payable = {{ $property_sell->block_sold_price }};
    // END ABHI SCRIPT
    var balance_amount = 0;
    var other_payment = 0;
    var down_payment = 0;
    var installment_amount = 0;
    var no_of_installment = 0;
    var loan_capital = 0;

    $(document).ready(function () {
        ($('#balance_amount').val() === '') ? balance_amount = 0 : balance_amount = parseInt($('#balance_amount').val());
        ($('#other_payment').val() === '') ? other_payment = 0 : other_payment = parseInt($('#other_payment').val());
        ($('#down_payment').val() === '') ? down_payment = 0 : down_payment = parseInt($('#down_payment').val());
        if (isNaN((balance_amount + other_payment) - down_payment)) {
            $('#loan_capital').val(0);
        } else {
            $('#loan_capital').val((balance_amount + other_payment) - down_payment);
        }

        ($('#installment_amount').val() === '') ? installment_amount = 0 : installment_amount = parseInt($('#installment_amount').val());
        ($('#no_of_installment').val() === '') ? no_of_installment = 0 : no_of_installment = parseInt($('#no_of_installment').val());
        ($('#loan_capital').val() === '') ? loan_capital = 0 : loan_capital = parseInt($('#loan_capital').val());

        if (isNaN((installment_amount * no_of_installment) - loan_capital)) {
            $('#total_interest').val(0);
        } else {
            $('#total_interest').val((installment_amount * no_of_installment) - loan_capital);
        }
    });
    $('#other_payment, #balance_amount, #down_payment').change(function () {

        console.log("Balance Amount:" + $('#balance_amount').val() + "Other Amount:" + $('#other_payment').val() +
            "Down Amount:" + $('#down_payment').val()
        );
        ($('#balance_amount').val() === '') ? balance_amount = 0 : balance_amount = parseInt($('#balance_amount').val());
        ($('#other_payment').val() === '') ? other_payment = 0 : other_payment = parseInt($('#other_payment').val());
        ($('#down_payment').val() === '') ? down_payment = 0 : down_payment = parseInt($('#down_payment').val());
        ...
        console.log("Balance Amount:" + balance_amount + "Other Amount:" + other_payment +
            "Down Amount:" + down_payment
        );
    ...
    $('#other_payment, #balance_amount, #down_payment').keyup(function () {
        ...
        // ABHI SCRIPT
        let total_payabale = parseInt($("#total_payable").val())
        console.log("Total Payable:" + total_payable + " Other Payment:" + other_payment);
        $("#total_payable").val(total_payable + other_payment);
        // END ABHI SCRIPT
        console.log("Balance Amount:" + balance_amount + "Other Amount:" + other_payment +
            "Down Amount:" + down_payment
        );
    ...
    $('#installment_amount, #no_of_installment, #loan_capital').keyup(function () {
