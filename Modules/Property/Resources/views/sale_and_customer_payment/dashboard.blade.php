                ...
                <div class=" pull-left">
                     @can('property.project_dashboard.customer_payments')
                        <a href="{{action('\Modules\Property\Http\Controllers\CustomerPaymentController@create')}}"
                            type="button" style="font-size: 13px;" class="btn btn-danger btn-flat pull-right m-8  btn-sm mt-10">
                    ...
                    @endcan
                        <a href="{{action('Auth\PropertyUserLoginController@logout', ['main_system' => true])}}" type="button"
                            style="font-size: 13px; background: #9900cc; color: #fff;"
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '#period_1', function() {
                ...
                if(filter == "today"){
                    ...
                    var start = "{{ $date_filters['this_fy']['start']}}";
                    var end = "{{ $date_filters['this_fy']['end']}}";
                }else{
                    var start = "{{ date('Y-m-d') }}";
                    var end = "{{ date('Y-m-d') }}";
                }
