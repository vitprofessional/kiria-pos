...
class SellLandBlockController extends Controller
{
    ...
    public function create()
    {
        ...
        $payment_options = PaymentOption::where('business_id', $business_id)->where('is_default',1)->pluck('payment_option', 'id');
        ...
        $customers = Contact::where('business_id', $business_id)->where('is_property', 1)->where('type','customer')->pluck('name', 'id')->toArray();
        ...
        $card_group_accounts = Account::leftjoin('account_groups', 'accounts.asset_type', 'account_groups.id')
            ->where('accounts.business_id', $business_id)
...
//         try { 
        ...
        if($current_monthly_sale > $monthly_max_sale_limit) {
            ...
            $ref_count = $this->productUtil->setAndGetReferenceCount('property_sell');
            
            //Generate reference number
            if (empty($transaction_data['ref_no'])) {
                
                $startingNo = PropertyStartingNo::where('business_id', $business_id)
                ->where('is_active',1)->orderBy('id','desc')->first();
                
                if(!empty($startingNo) && $ref_count < $startingNo->starting_no){
                    $ref_count = $startingNo->starting_no;
                }
                
                $ref_digits =  str_pad($ref_count, 4, 0, STR_PAD_LEFT);
                
                
                $ref = ReferenceCount::where('ref_type', 'property_sell')
                            ->where('business_id', $business_id)
                            ->first();
                if($ref->ref_count < $ref_count){
                ...
                $transaction_data['ref_no'] = !empty($startingNo) ? $startingNo->prefix."-".$ref_digits : 'PO-'.$ref_digits;
            }
            ...
            $ransaction_payments_values = array('transaction_id' => $transaction->id,'business_id' => $business_id,'amount' => $transaction_data['final_total'],'method' => $request->payment_method,'payment_option_id' => $request->payment_option,'paid_on' => $transaction_data['transaction_date'],'payment_ref_no' => $transaction_data['ref_no'],'created_by' => Auth::user()->id,'created_at' => date('Y-m-d H:i:s'));
            DB::table('transaction_payments')->insert($ransaction_payments_values);
