        ...
        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    {!! Form::label('card_pmt_type', __('petro::lang.type')).':' !!}
                    <select id="card_pmt_type" class="form-control">
                        <option value="one_by_one" @if($card_pmt_type != 'bulk') selected @endif>
                            {{ __('petro::lang.one_by_one') }}
                        </option>
                        <option value="bulk" @if($card_pmt_type == 'bulk') selected @endif>
                            {{ __('petro::lang.bulk') }}
                        </option>
                    </select>
                </div>
            ...
            <div class="col-md-2 card-fields" @if($card_pmt_type == 'bulk') hidden @endif>
                <div class="form-group">
                    {!! Form::label('slip_no', __('petro::lang.slip_no')) !!}
                    {!! Form::text('slip_no', null, [
            ...
            <div class="col-md-3 card-fields" @if($card_pmt_type == 'bulk') hidden @endif>
                <div class="form-group">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const slipNoField = document.querySelector('input[name="slip_no"]');
        const cardNoField = document.querySelector('input[name="card_no"]');

        const toggleFields = () => {
            if (dropdown.value === 'bulk') {
                cardFields.forEach(field => field.setAttribute('hidden', true));
            } else {
                cardFields.forEach(field => field.removeAttribute('hidden'));
            }
        };
