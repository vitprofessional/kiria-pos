...
<section class="content-header main-content-inner">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane @if(session('status.tab') == 'daily_pump_status') active @endif" id="daily_pump_status">
            @if(!empty($message)) {!! $message !!} @endif
            @include('petro::pump_operators.partials.daily_pump_status_admin')
        </div>
...
<script type="text/javascript">
    ...
    list_daily_collection_table = $('#list_daily_collection_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            var footer_sold_fuel_qty = sum_table_col($('#list_daily_collection_table'), 'footer_sold_fuel_qty');
            $('#dc_footer_sold_fuel_qty').text(footer_sold_fuel_qty);
            
            var footer_testing_qty = sum_table_col($('#list_daily_collection_table'), 'footer_testing_qty');
            $('#dc_footer_testing_qty').text(footer_testing_qty);
            
            var footer_sold_fuel_amount = sum_table_col($('#list_daily_collection_table'), 'footer_sold_fuel_amount');
...
$(document).ready( function(){
    ...
    $('#payment_summary_pump_operators, #payment_summary_payment_method, #payment_summary_date_range, #payment_summary_location_id, #payment_summary_shift_id').change(function(){
        pump_operators_payment_summary_table.ajax.reload();
    });
});
...
$(document).ready( function(){
    ...
    pump_operators_meters_with_payments_table = $('#pump_operators_meters_with_payments_table').DataTable({
        ...
        aaSorting: [[0, 'desc']],
        ajax: {
            url: '{{action('\Modules\Petro\Http\Controllers\PumpOperatorPaymentController@metersWithPayments')}}',
            data: function(d) {
...
$(document).ready( function(){
    ...
    $('#close_shift_location_id, #close_shift_pump_operators, #close_shift_pumps, #close_shift_payment_method, #close_shift_date_range,#closing_shift_id').change(function(){
...
function reloadClosingShift(){
    ...
    $.ajax({
        ...
        dataType: 'html',
        data: {'shift_id': $("#closing_shift_id").val()  },
        success: function(result) {
...
$(document).ready( function(){
    pump_operators_current_meter_table = $('#pump_operators_current_meter_table').DataTable({
        ...
        columns: [
            { data: 'date_and_time', name: 'date_and_time' },
            { data: 'location_name', name: 'business_locations.location_name' },
            { data: 'pump_no', name: 'pump_no' },
            { data: 'name', name: 'pump_operators.name' },
            { data: 'starting_meter', name: 'starting_meter' },
    ...
    $('#current_meter_pump_operators, #current_meter_pump_no, #current_meter_date_range,#current_meter_location_id').change(function(){
        pump_operators_current_meter_table.ajax.reload();
    });
});
...
$(document).ready( function(){
    ...
    $('#unload_stock_product_id, #unload_stock_tank_id, #unload_stock_date_range, #unload_stock_location_id').change(function(){
        pump_operators_unload_stock_table.ajax.reload();
    });
});
