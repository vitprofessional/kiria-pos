    ...
    public function saveNewDip(Request $request)
    {
        ...
        try {
            ...
            foreach($tank_id as $key => $one){
                ...
                $prev_DipReading = DipReading::where('business_id', $business_id)->latest()->first();
                $opening_stock = 0;
                $received_stock = 0;
                $sold_qty = 0;
                $testing_qty = 0;
                if (!empty($prev_DipReading)) {
                    $opening_stock = $prev_DipReading->fuel_balance_dip_reading;
                    $received_stock = $fuel_bal[$key];
                    $sold_qty = $opening_stock - $received_stock;
                }
                ...
                $opening_stock = $this->productUtil->num_f($opening_stock);
                $received_stock = $this->productUtil->num_f($received_stock);
                $sold_qty = $this->productUtil->num_f($sold_qty);
                $testing_qty = $this->productUtil->num_f($testing_qty);
                $balance_stock = $this->productUtil->num_f($curr_qty[$key]);
                $dip_stock = $this->productUtil->num_f($fuel_bal[$key]);
                $difference_Stock = $this->productUtil->num_f($fuel_bal[$key] - $curr_qty[$key]);
            }
