...
class PumpController extends Controller
{
    ...
    public function index()
    {
        ...
        $canEdit = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_petro_pumps_edit') || auth()->user()->can('superadmin')?true:false;
        $canDelete = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_petro_pumps_delete')||auth()->user()->can('superadmin') ?true:false;
                ...
                $fuel_tanks = Datatables::of($query)
                    ->addColumn(
                        ...
                        if($canEdit || $canDelete) {
                            if($canEdit){
                                $tmphtml =  '<button data-href="{{action(\'\Modules\Petro\Http\Controllers\PumpController@edit\', [$id])}}" data-container=".fuel_tank_modal" class="btn btn-primary btn-xs btn-modal edit_reference_button"><i class="fa fa-pencil-square-o"></i>' .trans("messages.edit").'</button>';
                            }
                            if($canDelete){
                                $tmphtml .='<a href="{{action(\'\Modules\Petro\Http\Controllers\PumpController@destroy\', [$id])}}" class="delete_pump_button btn btn-danger btn-xs"><i class="fa fa-trash"></i> '.trans("messages.delete").'</a>';
                            }
                        }  
                        return  $tmphtml;
                    })
        ...
        $enable_petro_management_testing 	= $this->moduleUtil->hasThePermissionInSubscription($business_id, 'pump_management_testing'); 
        $enable_petro_meter_reading 		= $this->moduleUtil->hasThePermissionInSubscription($business_id, 'meter_reading');
        $enable_petro_pump_dashboard 		= $this->moduleUtil->hasThePermissionInSubscription($business_id, 'pump_dashboard_opening');
        $enable_petro_pump_management 		= $this->moduleUtil->hasThePermissionInSubscription($business_id, 'pump_management');
    ...
    public function saveImport(Request $request)
    {
        ...
        return redirect('/petro/pump-management')->with('status', $output);
    }
