...

{
...

    {
        ...
        if (request()->ajax()) {
            ...
            $tanks_transaction_details = Datatables::of($sell_query)
                ...
                ->addColumn('balance_qty', function ($row) use ($business_details) {
                    ...
                    if(!empty($row->invoice_no)){
                        $settlement = Settlement::where('settlement_no',$row->invoice_no)->select('id')->first();
                        if(!empty($settlement)){
                            $meter_sale = MeterSale::where('settlement_no',$settlement->id)->select('testing_qty')->get();
                            if(!empty($meter_sale)){
                                foreach($meter_sale as $ms){
                                    $testing_qty += $ms->testing_qty;
                                }
                            }
                        }
                    }
                    $ob = $this->transactionUtil->getTankBalanceByDateInclude($row->fuel_tank_id,$row->created_at);
                    return $this->productUtil->num_f(($ob + $testing_qty), false, $business_details, true);
                ...
                ->editColumn('purchase_qty', function ($row) use ($business_details) {
                    ...
                    if ($row->type == 'opening_stock' || $row->type == 'sell' || $row->type == 'transfer_out' || ($row->purchase_qty == 0 && $row->type == 'stock_adjustment')) {
                        ...
                        if($testing_qty > 0){
                            return $this->productUtil->num_f($testing_qty, false, $business_details, true) . "<span class='label  bg-light-green' style='margin-left: 5px'>Testing</span>";
                        }else {
                            return '';
                        }
            ...
            return $tanks_transaction_details->rawColumns(['transaction_date', 'balance_qty', 'ref_no','purchase_order_no', 'purchase_qty'])
            ...
            if (request()->ajax()) {
                ...
                foreach($dates as $date){
                    foreach($query->get() as $tank){
                        ...
                        $tank->testing_qty = $this->transactionUtil->__totalTesting($business_id,$tank->start_date,$tank->end_date,$tank->id);
                ...
                $tanks_transaction_details = Datatables::of($tanks)
                    ...
                    ->editColumn('sold_qty', function($row) use($business_details){
                        ...
                        return number_format($row->sold_qty, 3, '.', ',');
                    })

                    ->editColumn('purchase_qty', function($row) use($business_details){
                        // return $this->productUtil->num_f(($row->purchase_qty + $row->testing_qty), false, $business_details, true);
                        return number_format(($row->purchase_qty + $row->testing_qty), 3, '.', ',');
                    })

                    ->editColumn('balance_qty', function ($row) use($business_details) {
                        return number_format(($row->starting_qty + $row->purchase_qty - $row->sold_qty + $row->testing_qty), 3, '.', ',');
