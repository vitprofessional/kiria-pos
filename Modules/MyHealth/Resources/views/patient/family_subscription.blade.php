...
<section class="content-header">
    <h1>@lang('myhealth::patient.family_subscription')</h1>
</section>
        ...
        <div class="col-md-12">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {!! Form::select('no_of_family_members', $package_variables, null, ['class' => 'form-control', 'id' => 'no_of_family_members', 'placeholder' => __('patient.no_of_family_members')]); !!}
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
            ...
            <div class="row">
                <div class="col-md-12">
                    <button type="submit" disabled id="pay-submit"
                        class="pull-right btn btn-success">@lang('myhealth::patient.pay')</button>
                </div>
            </div>
            {!! Form::close() !!}
        </div>
...
<script>
    ...
    $('#no_of_family_members').change(function(event) {
        ...
        $.ajax({
            ...
            success: function(result) {
                if(result){
                    ...
                    amount_to_pay = 0;
                    const packagePrice = parseFloat({{ $package ? $package->price : 0 }}),
                    oldPackagePrice = parseFloat({{ $old_package ? $old_package->price : 0 }});
                    if(result.variable_type == '1') { // type percentaee
                        amount_to_pay = (packagePrice > 0) ? (price_value * packagePrice) : (price_value * oldPackagePrice);
                        amount_to_pay = amount_to_pay / 100;
                        $('#type').val('Percentage');
                    }
                    if(result.variable_type == '0') { // type fixed
                        amount_to_pay = (packagePrice > 0) ? price_value : (price_value + oldPackagePrice);
                        $('#type').val('Fixed');
                    }
