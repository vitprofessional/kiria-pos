...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	<table class="table table-condensed text-left align-left no-border @if(!empty($for_pdf)) table-pdf @endif">
		...
		<tr>
			<td><strong>@lang('airline::lang.total_commission')</strong></td>
			<td><span id="total_commission">0.00</span></td>
		</tr>
		<tr>
			<td><strong>@lang('airline::lang.total_payments')</strong></td>
...
<div class="table-responsive">
    <table class="table table-bordered table-striped" id="rp_log_table">
        <thead>
            <tr>
                <th>@lang('airline::lang.date')</th>
                <th>@lang('airline::lang.description')</th>
                <th>@lang('airline::lang.type')</th>
                <th>@lang('airline::lang.debit')</th>
                <th>@lang('airline::lang.credit')</th>
                <th>@lang('airline::lang.balance')</th>
                <th>@lang('airline::lang.payment_method')</th>
            </tr>
        </thead>
        <tbody>
            @php $balance = $ledger_details['beginning_balance']; $commission = 0; $payment = 0;$ob = $agent->opening_balance; @endphp
            ...
            @endphp
                ...
                @elseif($one->type == 'agent_payment' || $one->type == 'shipping_agent_ob')
                    @php $balance -= $one->amount;
                    
                    if($one->type == 'shipping_agent_ob'){
                        $ob += $one->amount; 
                    }else{
                        $payment += $one->amount; 
                    }
                    ...
                    @endphp
                @elseif ($one->type == 'airline_ticket')
                @php $balance += $one->amount; 
                $payment += $one->amount; @endphp
                 
                @endif
                
                @php
                    if(in_array($one->type,['commission','agent_payment','airline_ticket']) && !empty($one->parent_id)){
                ...
                <tr>
                    ...
                    <td>
                        @if(!empty($parent))
                            <b>@lang('airline::lang.tracking_no')</b>: {{ $parent->tracking_no }}<br>
                            @if(!empty($customer))
                                <b>@lang('airline::lang.customer')</b>: {{$customer->name }}<br>
                            @endif
                            @if(!empty($recipient))
                                <b>@lang('airline::lang.recipient')</b>: {{$recipient->name }}<br>
                            @endif
                            @if(!empty($parent->delivery_time))
                                <b>@lang('airline::lang.delivery_time')</b>: {{ @format_date($parent->delivery_time) }}<br>
                            @endif
                    ...
                    <td>
                        @if($one->type == 'commission')
                            {{__('airline::lang.commission')}}
                        @elseif($one->type == 'agent_payment')
                            {{__('airline::lang.airline_agent_payment')}}
                        @elseif($one->type == 'airline_ticket')
                        {{__('airline::lang.airline_agent_payment')}}
                        @elseif($one->type == 'shipping_agent_ob')
                            {{__('airline::lang.opening_balance')}}
                        @endif
                    </td>
                    <td>
                        @if($one->type == 'agent_payment' || (in_array($one->type ,['shipping_agent_ob','airline_ticket']) && $one->amount < 0))
                            {{@num_format(abs($one->amount))}}
                        @endif
                    </td>
                    <td>
                        @if($one->type == 'commission'  || (in_array($one->type ,['shipping_agent_ob','airline_ticket']) && $one->amount > 0))
                            {{@num_format(abs($one->amount))}}
                        @endif
                    </td>
                    <td>
