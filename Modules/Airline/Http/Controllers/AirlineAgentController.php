...
/**
 * Doc:6787 Airlines & Agents
 * Dev:Sakhawat Kamran 
 */
class AirlineAgentController extends  Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $drivers = AirlineAgent::leftjoin('users', 'airline_agents.user_id', 'users.id')
            ...
            if (!empty(request()->name)) {
                $drivers->where('agent', request()->name);
            }
            ...
            return DataTables::of($drivers)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        // $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@addCommission', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="fa fa-credit-card"></i> ' . __("shipping::lang.add_commission") . '</a></li>';
                        
                        // $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@viewCommissions', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="fa fa-eye"></i> ' . __("shipping::lang.view_commission") . '</a></li>';
                        // $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@addPayment', [$row->id]) . '" class="btn-modal" data-container=".view_modal"> + ' . __("shipping::lang.add_payment") . '</a></li>';
                        
                        // $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@shipmentDetails', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __("shipping::lang.shipping_details") . '</a></li>';
                        // $html .= '<li><a href="#" data-href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</a></li>';
                        $html .= '<li><a href="' . action('\Modules\Airline\Http\Controllers\AirlineAgentController@show', [$row->id]) . '?tab=ledger" class=""><i class="fa fa-anchor"></i> ' . __("lang_v1.ledger") . '</a></li>';

                        return $html;
                    }
                )
                ->addColumn('current_due',function($row){
                    $due = $this->transactionUtil->num_f($this->transactionUtil->__getAgentBalance($row->id,'airline'));
                ...
                ->addColumn('mobile_1',function($row){
                    return $row->mobile_1??"N/A";
                })
                ->addColumn('mobile_2',function($row){
                    return $row->mobile_2??"N/A";
                })
                ->addColumn('land_no',function($row){
                    return $row->land_no??"N/A";
                })
        ...
        $name = AirlineAgent::where('business_id',$business_id)->select('agent as name')->pluck('name','name');
        
        $mobile_1 = AirlineAgent::where('business_id',$business_id)->select('mobile_1')->pluck('mobile_1','mobile_1');
        $mobile_2 = AirlineAgent::where('business_id',$business_id)->select('mobile_2')->pluck('mobile_2','mobile_2');
        ...
        return view('airline::agents.index',compact('mobile','name'));
    }
    ...
    public function viewLedger($id){
        ...
        $ledger_transactions = $this->transactionUtil->__getAgentLedger($id,$start_date,$end_date,'airline');
        $ledger_details = [
                'start_date' => $start_date,
                'end_date' => $end_date,
                'beginning_balance' => $this->transactionUtil->__getAgentBFBalance($id,$start_date,'airline')
            ];
        
        return view('airline::agents.partials.ledger_tab')->with(
            compact(
    ...
    public function shippingDetails($id)
    {
        ...
        return view('airline::agents.shipping_details');
    }
     ...
     public function shipmentDetails($id)
    {
        ...
        return view('airline::agents.shipping_details')->with(compact(
            'id'
        ));
    }
    ...
    public function create()
    {
        ...
        return view('airline::agents.create')->with(compact(
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $agent = AirlineAgent::create($data);
            
            if($request->opening_balance != 0){
                $transaction_data = [];
                $transaction_data['parent_transaction_id'] = $agent->id;
                ...
                $transaction_data['type'] = 'airline_agent_ob';
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($shipment)
                ...
                ->editColumn('payment_status',function($row){
                    $html =  (string) view('airline::shipping.payment_status', ['payment_status' => $row->payment_status]);
                    return $html;
                })
                ->removeColumn('id')
    ...
    public function viewCommissions($id){
        ...
        return view('airline::agents.viewCommission')->with(compact(
            'business_id','id'
        ));
    ...
    {
        $business_id = request()->session()->get('business.id');
        // $driver_dropdown = AirlineAgent::where('business_id', $business_id)->pluck('agent', 'id');
        $driver_dropdown = AirlineAgent::pluck('agent', 'id');
        $view_type = request()->tab;
        $driver = AirlineAgent::find($id);
        

        return view('airline::agents.show')->with(compact(
            'driver_dropdown',
            'view_type',
            'driver',
            'id'
    ...
    public function edit($id)
    {
        ...
        return view('airline::agents.edit')->with(compact(
            'driver'
        ));
    }
    ...
    public function addCommission($id)
    {
        ...
        return view('airline::agents.addCommission')->with(compact(
            'agent','shipments' 
        ));
    }
    ...
    public function addPayment($id)
    {
        ...
        return view('airline::agents.addPayment')->with(compact(
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $data['joined_date'] = $this->commonUtil->uf_date($data['joined_date']);
            $agent = AirlineAgent::findOrFail($id);
            AirlineAgent::where('id', $id)->update($data);
            $transaction = Transaction::where('type','shipping_agent_ob')->where('parent_transaction_id',$agent->id)->first();
