        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($purchases)
                ...
                ->addColumn('stock_count', function ($row) use($qty_precision, $business_id) {
                  ...
                  $stockCount = number_format($row->alert_quantity, $precision); 

                return '<input class="form-control stock_count"  name="f22[' . $row->product_id . '][stock_count]" id="f22[' . $row->product_id . '][stock_count]"  style="width: 80px;" name="stock_count" value="'.$stockCount.'" >';
                })
                ->addColumn('qty_difference', function ($row) {
                    return '<input class="form-control qty_difference" name="f22[' . $row->product_id . '][qty_difference]" id="f22[' . $row->product_id . '][qty_difference]" style="width: 65px;" name="qty_difference" value="" readonly >';
                })
                ->editColumn('sku', function ($row) {
        ...
        try {
            ...
            foreach ($data['f22'] as $key => $item) {
                ...
                if (empty($item['stock_count'])) {
                    ...
                    $difference = (float)$item['stock_count'] - (float)$item['current_stock'];
                }
