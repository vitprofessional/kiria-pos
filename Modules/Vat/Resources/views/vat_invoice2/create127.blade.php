...
@section('content')
    ...
    <div class="col-md-12">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {!! Form::date('supplied_on', null, ['class' => 'form-control', 'placeholder' => __('vat::lang.supplied_on')]) !!}
                </div>
            </div>
            ...
            {!! Form::hidden('delivery_to[name]', null, ['id' => 'delivery_to_name']) !!}
            {!! Form::hidden('delivery_to[address]', null, ['id' => 'delivery_to_address']) !!}

            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('delivery_to', __('vat::lang.delivery_to')) !!}
                    <button class="form-control btn btn-danger btn-modal" data-target="#deliveryToModal"
                        id="delivery_button">
                        {{ __('vat::lang.click_here_to_enter') }}
                    </button>
        ...
        <div class="pull-right">
            <button type="submit" class="btn btn-primary" id="save_issue_bill_customer_btn"
                formaction="{{ action('\Modules\Vat\Http\Controllers\VatInvoice2Controller@store127') }}">@lang('messages.save')</button>
            <button type="submit" class="btn btn-danger" id="save_issue_bill_customer_btn"
                formaction="{{ action('\Modules\Vat\Http\Controllers\VatInvoice2Controller@store127') }}?is_print=true">@lang('messages.save_and_print')</button>
        </div>
    ...
    <div class="modal fade delivery_to_modal" id="deliveryToModal" tabindex="-1" role="dialog"
        aria-labelledby="gridSystemModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="gridSystemModalLabel">Add Delivery Address</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="p-3">
                        <div class="form-group">
                            {!! Form::label('name', __('vat::lang.name')) !!}
                            {!! Form::text('name', null, [
                                'class' => 'form-control',
                                'required',
                                'placeholder' => __('vat::lang.enter_name'),
                            ]) !!}
                        </div>
                        <div class="form-group">
                            ...
                            {!! Form::textarea('address', null, [
                                'class' => 'form-control',
                                'required',
                                'placeholder' => __('vat::lang.enter_address'),
                            ]) !!}
                        </div>
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" data-dismiss="modal"
                                id="saveDeliveryToButton">Save</button>
                        </div>
                    </div>
                    {!! Form::close() !!}
    ...
    <script src="{{url('Modules/Vat/Resources/assets/js/app-new.js')}}"></script>
        <script>
            var deliveryButton = document.getElementById("delivery_button");
            deliveryButton.onclick = function() {
                var modal = document.getElementById('deliveryToModal');
                $(modal).modal('show');
            };
            ...
            var deliveryToNameInput = document.getElementById('delivery_to_name');
            var deliveryToAddressInput = document.getElementById('delivery_to_address');

            document.getElementById('saveDeliveryToButton').addEventListener('click', function() {
                var name = nameInput.value.trim();
                var address = addressInput.value.trim();

                deliveryToNameInput.value = name;
                deliveryToAddressInput.value = address;
                if (name !== '' && address !== '') {
        ...
        `;
                ...
                function calculate($this) {
                    var unitPriceField = $($this).closest('tr').find('.unit_price_excl');
                    ...
                    var unit_vat = ((discountedUnitPrice * tax_rate) / 100).toFixed(2); // Calculate unit VAT

                    var unit_vat_tax = (unitPrice * tax_rate) / 100;
                    var vat = parseFloat(unit_vat) * qty; // Calculate total VAT for the line item

                    var subTotal = (discountedUnitPrice + parseFloat(unit_vat)) *
