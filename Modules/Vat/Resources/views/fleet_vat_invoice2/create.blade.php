...
<div class="col-md-12">
    ...
    <div class="col-md-3">
        <div class="form-group">
          ...
          {!! Form::text('customer_bill_no', $bill_no, ['class' => 'form-control', 'required','readonly', 'placeholder' =>
          __( 'vat::lang.bill_no')]);
          !!}
        </div>
      </div>
    ...
    <table class="table table-responsive" id="issue_customer_bill_add_table">
      ...
      <tbody>
        <tr class="first-row">
          <td>
            {!! Form::hidden('issue_customer_bill[product_id][]', null, ['required', 'id' => 'product_id','class' => 'product_id']) !!}
            {!! Form::text('product_name',null, ['class' => 'form-control product_name', 'style' => 'width:100%;', 'readonly', 'placeholder' =>
            __( 'vat::lang.product')]) !!}
          </td>
          <td>
    ...
    <div class="pull-right">
      <button type="submit" class="btn btn-primary" id="save_issue_bill_customer_btn" formaction="{{action('\Modules\Vat\Http\Controllers\FleetVatInvoice2Controller@store')}}">@lang( 'messages.save' )</button>
      <button type="submit" class="btn btn-danger" id="save_issue_bill_customer_btn"  formaction="{{action('\Modules\Vat\Http\Controllers\FleetVatInvoice2Controller@store')}}?is_print=true">@lang( 'messages.save_and_print' )</button>
    </div>
...
<script>
  ...
  $(document).on('change','#ro_id',function(){
    ...
    if(customer_id){
        $.ajax({
            ...
            success: function (result) {
                ...
                $('.first-row').find('.product_id').val(result.id).trigger('change');
                $('.first-row').find('.product_name').val(result.invoice_no);
                $('.first-row').find('.unit_price').val(result.amount);
                $('.first-row').find('.qty').val(1).trigger('change');
