...
class RepaymentCreatedUpdateTransactions
{
    /**
    ...
    public function handle(RepaymentCreated $event)
    {
        ...
        $to_update = [];
        //check if we have any transactions ahead of this current one
        if (LoanTransaction::where('loan_id', $loan->id)->where('transaction_type',
                'repayment')->where('reversed', 0)->where('date', '>', $loan_transaction->date)->count() > 0) {
            //reset loan schedules
            ...
            $fees_paid = 0;
            $penalty_paid = 0;
            foreach (LoanSchedule::where('loan_id', $loan->id)->orderBy('id', 'asc')->get() as $key) {
                $principal_due = $key->principal - $key->principal_paid - $key->principal_waived;
                $interest_due = $key->interest - $key->interest_paid - $key->interest_waived;
                $fees_due = $key->fees - $key->fees_paid - $key->fees_waived;
                $penalty_due = $key->penalty - $key->penalty_paid - $key->penalty_waived;
        ...
        event(new RepaymentNotification($event->loan_transaction));
    }
}
