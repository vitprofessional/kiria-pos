        ...
        @can('product.view')
            <div class="row">
                ...
                @component('components.widget')
                    @slot('title')
                        {{ trans_choice('core.edit', 1) }} {{ trans_choice('loan::general.purpose', 1) }}
                    @endslot

                    @slot('slot')
                        <section class="content">
                            <form method="post" action="{{ url('contact_loan/purpose/' . $loan_purpose->id . '/update') }}">
                                {{ csrf_field() }}
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label for="name" class="control-label">{{ trans_choice('core.name', 1) }}
                                                        @show_tooltip(__('loan::lang.tooltip_loanpurposessettingsname'))</label>
...
@section('tab-javascript')
    <script>
        var app = new Vue({
            el: "#vue-app",
            data: {
                name: "{{ old('name', $loan_purpose->name) }}",
            }
        })
    </script>
@endsection
