        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->addColumn('debit', function ($row) use ($business_details, $currency_precision) {
                    if ($row->type == 'debit') {
                        return  '<span class="display_currency debit_col" data-currency_symbol=false data-orig-value="' . $row->amount . '" >' . $this->productUtil->num_f($row->amount) . '</span>';
                    }
                    return '';
                })
                ->addColumn('credit', function ($row) use ($business_details, $currency_precision) {
                    if ($row->type == 'credit') {
                        return  '<span class="display_currency credit_col" data-currency_symbol=false data-orig-value="' . $row->amount . '" >' . $this->productUtil->num_f($row->amount) . '</span>';
                    }
                    return '';
                })
                ...
                ->rawColumns(['credit', 'debit', 'balance', 'mass_delete', 'sub_type', 'action', 'attachment', 'reconcile_status', 'description'])
                ->make(true);
        }
    }
        ...
        try {
            ...
            if (!empty($request->input('selected_rows'))) {
                ...
                foreach ($selected_rows as $id) {
                    ...
                    $business_id = $account_transaction->business_id;
