...
{   
       ...
       if (request()->ajax()) {
            ...
            return DataTables::of($job_sheets)
                ...
                ->editColumn('status', function($row) {
                    $html = '<a data-href="' . action("\Modules\AutoRepairServices\Http\Controllers\JobSheetController@editStatus", [$row->id]) . '" class="edit_job_sheet_status cursor-pointer" data-orig-value="'.$row->status.'" data-status-name="'.$row->status.'">
        ...
        $vehicle=new_vehicle::vehicleDetails();
    ...
    {   
        ...
        return view('autorepairservices::job_sheet.create')
            ->with(compact('status','repair_statuses', 'device_models', 'brands', 'devices', 'default_status', 'technecians', 'business_locations', 'types', 'customer_groups', 'walk_in_customer', 'repair_settings', 'contact_id', 'vehicals', 'last_inserted_vehicel_id'));
    }
    ...
    error_log('Error: ' . $e->getMessage()); // Logs error to server log
    //echo "An error occurred: " . $e->getMessage(); // Display error message (optional)
}
