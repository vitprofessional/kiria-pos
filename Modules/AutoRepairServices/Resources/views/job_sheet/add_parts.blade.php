...
<section class="content">
	...
	@endcomponent
	{!! Form::open(['url' => action('\Modules\AutoRepairServices\Http\Controllers\JobSheetController@saveParts', $job_sheet->id), 'method' => 'post', 'id' => 'add_part_form' ]) !!}
					...
					<tbody>
						@if(!empty($parts))
							@foreach($parts as $part)
								@include('autorepairservices::job_sheet.partials.job_sheet_part_row', ['variation_name' => $part['variation_name'], 'unit' => $part['unit'], 'quantity' => $part['quantity'], 'variation_id' => $part['variation_id']])
							@endforeach
						@endif
					</tbody>
				</table>
	...
	@if(!empty($status_update_data) && $status_update_data['job_sheet_id'] == $job_sheet->id)
		@component('components.widget', ['class' => 'box-solid'])
			@include('autorepairservices::job_sheet.partials.edit_status_form', ['status_update_data' => $status_update_data])
		@endcomponent
	@endif
	<div class="row">
		<div class="col-sm-12">
