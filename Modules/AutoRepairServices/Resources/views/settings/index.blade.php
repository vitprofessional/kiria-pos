...
@section('css')
<link rel="stylesheet" href="{{ asset('plugins/bootstrap-colorpicker/bootstrap-colorpicker.min.css') }}">
@endsection
<!-- Main content -->
                ...
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#repair_status_tab" data-toggle="tab" aria-expanded="true">
                            ...
                            @lang('sale.status')
                            @show_tooltip(__('autorepairservices::lang.all_js_status_tooltip'))
                        </a>
                    </li>
                    <li>
                        <a href="#repair_device_tab" data-toggle="tab" aria-expanded="true">
                            <i class="fas fa fa-desktop"></i>
                            @lang('autorepairservices::lang.devices')
                            @show_tooltip(__('autorepairservices::lang.device_tooltip'))
                        </a>
                    </li>
                    <li>
                        <a href="#repair_device_models_tab" data-toggle="tab" aria-expanded="true">
                            <i class="fas fa fa-bolt"></i>
                            @lang('autorepairservices::lang.device_models')
                            @show_tooltip(__('autorepairservices::lang.device_models_tooltip'))
                        </a>
                    </li>
                    <li>
                        <a href="#repair_settings_tab" data-toggle="tab" aria-expanded="true">
                            <i class="fa fas fa-cogs"></i>
                            @lang('autorepairservices::lang.repair_settings')
                        </a>
                    </li>
                </ul>
                <div class="tab-content">
                    <div class="tab-pane active" id="repair_status_tab"> 
                        @includeIf('autorepairservices::status.index')
                    </div>
                    ...
                    <!-- Device (Taxonomy)-->
                    <!--<input type="hidden" name="category_type" id="category_type" value="auto-device">-->
                    <!--<div class="tab-pane taxonomy_body" id="repair_device_tab">-->
                    <!--</div>-->
                    <!-- /Device (Taxonomy)-->
			        <div class="tab-pane" id="repair_device_tab"> 
                         @includeIf('autorepairservices::device.index')
                    </div>
                    ...
                    <div class="tab-pane" id="repair_device_models_tab">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    {!! Form::label('device_id',  __('autorepairservices::lang.device') . ':') !!}
                                    {!! Form::select('device_id', $devices, null, ['class' => 'form-control select2', 'style' => 'width:100%', 'placeholder' => __('lang_v1.all')]); !!}
                                </div>
                            </div>
                        </div>
                        @includeIf('autorepairservices::device_model.index')
                    </div>
                    <div class="tab-pane" id="repair_settings_tab"> 
                        @includeIf('autorepairservices::settings.partials.repair_settings_tab')
                    </div>
                </div>
            </div>
        </div>
...
$(document).on('submit', '#category_edit_form', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
               // alert(response.msg); // Show success message (optional)
                $('#editCategoryModal').modal('hide'); // Close Bootstrap modal
        ...
        error: function(xhr) {
           // alert("Something went wrong!"); // Handle unexpected errors
        }
    });
});
        ...
        var status_table = $('#status_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{action('\Modules\AutoRepairServices\Http\Controllers\RepairStatusController@index')}}",
                aaSorting: [[2, 'desc']],
                columnDefs: [ {
                    "targets": 3,
                    "orderable": false,
