    ...
    public function postRepairStatus(Request $request)
    {
        if ($request->ajax()) {
            try {
                ...
                }elseif (!empty($search_type) && $search_type == 'vehicle_no') {
                    $query->where('new_vehicle.vehicle_no', $search_number);
                }
                ...
                foreach ($sells as $key => $sell) {
                    $sells[$key]['activities'] = Activity::forSubject($sell)
                                           ...
                                           ->latest()
                                           ->get();
                    $parts = $sells[$key]->parts;
                    $partsAdded = [];
                    foreach($parts as $onekey => $value){
                        $name = DB::table('products')->where('id',$onekey)->first()->name;
                        $partsAdded[] = array("name" => $name, "quantity" => $value['quantity']);
                    }
                    $sells[$key]['added_parts'] = $partsAdded;
                }
