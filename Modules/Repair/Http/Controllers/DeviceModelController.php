            ...
            return Datatables::of($models)
                    ->addColumn('action', function ($row) {
                        ...
                        $html .= '<ul class="dropdown-menu dropdown-menu-left" role="menu">
                                <li>
                                    <a data-href="' . action('\Modules\Repair\Http\Controllers\DeviceModelController@edit', ['device_model' => $row->id]) . '" class="cursor-pointer edit_device_model">
                                        <i class="fa fa-edit"></i>
                                        '.__("messages.edit").'
                                    </a>
                                </li>
                                <li>
                                    <a data-href="' . action('\Modules\Repair\Http\Controllers\DeviceModelController@destroy', ['device_model' => $row->id]) . '"  id="delete_a_model" class="cursor-pointer">
...
public function device(Request $request)
    {
        ...
        if ($request->ajax()) {
            $models = DeviceModel::with('Device', 'Brand')
             ...
             $category = Category::where('category_type', $category_type)->select('*')->get();


            return Datatables::of($category)
                    ->addColumn(
                    'action',
                    '<button data-href="{{action(\'\App\Http\Controllers\TaxonomyController@edit_device\', [$id])}}" class="btn btn-xs btn-primary btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>'
                )
