...
use Modules\Stocktaking\Entities\FormOpeningValue;
...
class FormsSettingController extends Controller
{
    ...
    public function index()
    {
        ...
        $Stocktaking_form_settings_permission = 1; //$this->moduleUtil->hasThePermissionInSubscription($business_id, 'Stocktaking_form_settings');
        $list_opening_values_permission = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'list_opening_values');

        return view('Stocktaking::forms_setting.index')->with(compact('expense_accounts','income_accounts','business_locations', 'settings', 'Stocktaking_form_settings_permission', 'list_opening_values_permission'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            StocktakingFormSetting::updateOrCreate(['business_id' => $business_id], $input);

            $output = [
                'success' => true,
                'msg' => __('Stocktaking::lang.settings_update_success')
            ];
        } catch (\Exception $e) {
    ...
    public function getForm9CSetting()
    {
        ...
        $settings = StocktakingFormSetting::where('business_id', $business_id)->first();
        $months = StocktakingFormSetting::getMonthArray();

        return view('Stocktaking::forms_setting.partials.form_9c_modal')->with(compact('sub_categories', 'settings', 'business_id', 'months'));
    }
    public function postForm9CSetting(Request $request)
    {
        try {
            ...
            $setting = StocktakingFormSetting::where('business_id', $business_id)->update($data);
            $save_sub_cat_data = [];
            if (!empty($request->sub_cat_9c)) {
                ...
                $form_id = StocktakingFormSetting::where('business_id', $business_id)->first();
    ...
    {
        $business_id = request()->session()->get('business.id');
        $months_numbers = StocktakingFormSetting::getMonthArray();

        $setting = StocktakingFormSetting::where('business_id', $business_id)->first();

        return view('Stocktaking::forms_setting.partials.form_16a_modal')->with(compact('months_numbers', 'setting'));
    }
    public function postForm16ASetting(Request $request)
    {
        try {
            ...
            StocktakingFormSetting::where('business_id', $business_id)->update($data);
            $form_id = StocktakingFormSetting::where('business_id', $business_id)->first();
            FormOpeningValue::create([
                'business_id' => $business_id,
                'form_name' => 'F16A',
    ...
    public function getFormF22Setting()
    {
        ...
        return view('Stocktaking::forms_setting.partials.form_f22_modal')->with(compact('settings'));
    }
    public function postFormF22Setting(Request $request)
    {
        ...
        $output = [
            'success' => 1,
            'msg' => __('Stocktaking::lang.setting_save_success')
        ];
    ...
    public function getForm159ABCSetting()
    {
        ...
        return view('Stocktaking::forms_setting.partials.form_15_9_abc_modal')->with(compact('months', 'setting'));
    }
    ...
    public function getForm21CSetting()
    {
        ...
        return view('Stocktaking::forms_setting.partials.form_21c_modal')->with(compact('merged_sub_categories', 'settings', 'months'));
    }
