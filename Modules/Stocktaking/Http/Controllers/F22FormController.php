...
class F22FormController extends Controller
{
    ...
    public function F22StockTaking()
    {
        ...
        $products = Product::where('business_id', $business_id)->forModule('stocktaking_stocktaking')->pluck('name', 'id');

        $settings = StocktakingFormSetting::where('business_id',  $business_id)->select('F22_no_of_product_per_page')->first();
        ...
        return view('Stocktaking::forms.F22.F22_stock_taking')->with(compact('F22_from_no', 'business_locations', 'products', 'settings', 'last_form_no'));
    }
            ...
            return Datatables::of($header)
                ...
                ->editColumn('action', function ($row) {
                    ...
                    if (auth()->user()->can("edit_f22_stock_Taking_form")) {
                        $html .= '<li><a href="' . action('\Modules\Stocktaking\Http\Controllers\F22FormController@edit', [$row->id]) . '"><i class="glyphicon glyphicon-edit" aria-hidden="true"></i>' . __("messages.edit") . '</a></li>';
                    }

                    $html .= '<li><a href="#" class="reprint_form" data-href="' . action('\Modules\Stocktaking\Http\Controllers\F22FormController@printF22FormById', [$row->id]) . '"><i class="fa fa-print" aria-hidden="true"></i>' . __("messages.print") . '</a></li>';
    ...
    public function edit($id)
    {
        ...
        return view('Stocktaking::forms.F22.edit')->with(compact('id'));
    }
    ...
    public function getF22Form(Request $request)
    {
        ...
        if (request()->ajax()) {
            $purchases = Product::leftjoin('purchase_lines', 'products.id', 'purchase_lines.product_id')
                ...
                )->forModule('stocktaking_stocktaking')
            ...
            return DataTables::of($purchases)
                ...
                ->addColumn('difference_value', function ($row) {
                    return '<input class="form-control difference_value" name="f22[' . $row->product_id . '][difference_value]" id="f22[' . $row->product_id . '][difference_value]" style="width: 80px;" name="difference_value" value="" readonly >';
                })
                ->editColumn('sku', function ($row) {
                ...
                ->rawColumns(['total_purchase_price', 'total_sale_price', 'book_no', 'stock_count', 'qty_difference', 'unit_purchase_price', 'unit_sale_price', 'current_stock', 'sku', 'product','difference_value'])
                ->make(true);
        }
    }
    ...
    public function printF22Form(Request $request)
    {
        ...
        $details = $data;
        $data = $data['f22'];
        return view('Stocktaking::forms.F22.partials.print_f22_form')->with(compact('data', 'settings', 'details'));
    }
    ...
    public function printF22FormById($header_id)
    {
        ...
        return view('Stocktaking::forms.F22.partials.print_byID_f22_form')->with(compact('header', 'details', 'settings'));
    }
            ...
            foreach ($data['f22'] as $key => $item) {
                ...
                if (!empty($settings)) {
                    if ($settings->current_stock_aa_onstocktaking == 1) {
                        ...
                        if($item['difference_value'] > 0 || $item['difference_value'] < 0){
                            
                            if(($item['difference_value'] > 0 && $settings->stock_adjustment_gain_account > 0) || ($item['difference_value'] < 0 && $settings->stock_adjustment_loss_account > 0)){
                                    if($item['difference_value'] > 0){
                                        $type = "stock_taking";
                                        $account = $settings->stock_adjustment_gain_account;
                                        $amount = $item['difference_value'];
                                        
                                        $t1 = "credit";
                                        $t2 = "debit";
                                        
                                    }elseif($item['difference_value'] < 0){
                                        $type = "stock_taking";
                                        $account = $settings->stock_adjustment_loss_account;
                                        $amount = -1* ((int) $item['difference_value']);
