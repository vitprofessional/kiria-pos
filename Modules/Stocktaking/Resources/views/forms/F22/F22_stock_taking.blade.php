@extends('layouts.app')
@section('title', __('Stocktaking::lang.F22StockTaking_form'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1> @lang('Stocktaking::lang.F22StockTaking_form')
        <small>@lang( 'Stocktaking::lang.F22StockTaking_form', ['contacts' => __('Stocktaking::lang.mange_F22StockTaking_form')
            ])</small>
    </h1>
</section>
            ...
            <div class="settlement_tabs">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#f22_form_tab" class="f22_form_tab" data-toggle="tab">
                            <i class="fa fa-file-text-o"></i> <strong>@lang('Stocktaking::lang.f22_form')</strong>
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="#f22_last_verified_stock_tab" class="f22_last_verified_stock_tab" style=""
                            ...
                            <i class="fa fa-check"></i> <strong>
                                @lang('Stocktaking::lang.f22_last_verified_stock') </strong>
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="#list_f22_stock_taking_tab" class="list_f22_stock_taking_tab" style=""
                            ...
                            <i class="fa fa-sign-in"></i> <strong>
                                @lang('Stocktaking::lang.list_f22_stock_taking') </strong>
                        </a>
                    </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="f22_form_tab">
                        @include('Stocktaking::forms.F22.partials.f22_form')
                    </div>

                    <div class="tab-pane" id="f22_last_verified_stock_tab">
                        @include('Stocktaking::forms.F22.partials.f22_last_verified_stock')
                    </div>

                    <div class="tab-pane" id="list_f22_stock_taking_tab">
                        @include('Stocktaking::forms.F22.partials.list_f22_stock_taking')
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready(function(){
     form_f22_list_table = $('#form_f22_list_table').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: '/Stocktaking/get-form-f22-list',
            data: function(d) {
    ...
    $(document).on('keyup', '.stock_count', function(){
       ...
       let difference_value = qty_difference * unit_purchase_price;
       ...
       tr.find('.difference_value').val(difference_value);
    ...
    var lf_pre_gspage_totals = [];
     form_22_last_verified_table = $('#form_22_last_verified_table').DataTable({
        ...
        pageLength: {{!empty($settings->F22_no_of_product_per_page) ? $settings->F22_no_of_product_per_page : 25}},
        ajax: {
            url: '/Stocktaking/get-last-verified-form-f22',
            data: function(d) {
            }
        },
        "columnDefs": [
    ...
    $('#f22_print').click(function(e){
        ...
        $.ajax({
            method: 'post',
            url: '/Stocktaking/print-form-f22',
            data: { data: form_22_table.$('input, select').serialize() + $('#f22_form').serialize() },
            success: function(result) {
                if(result.success == 0){
                    toastr.error(result.msg);
...
function printPage(content) {
		...
		w.print();
		w.close();
        window.location.href = "{{URL::to('/')}}/Stocktaking/F22_stock_taking";
	}
