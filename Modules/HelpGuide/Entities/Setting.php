...
class Setting extends BaseModel
{
     ...
     * @return array
     */
    public static function getValidationRulesFor(array $fields)
    {
      return self::getDefinedSettingFields()->pluck('rules', 'name')
          ->reject(function ($val, $key) use ( $fields ){
          return is_null($val) || !in_array($key, $fields);
      })->toArray();
    }
     ...
     * @return mixed
     */
    public static function isFieldExists($field)
    {
        $type  = self::getDefinedSettingFields()
                ->pluck('data', 'name')
                ->get($field);
        return is_null($type) ? false : true;
    }
    ...
    {
        return collect(config('helpguide.setting_fields'))->pluck('elements')->flatten(1);
    }
    ...
    {
        if (!isAppInstalled()) return collect();
