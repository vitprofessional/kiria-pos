...
<div class="d-flex justify-content-end options mb-3">
    <button class="btn btn-sm btn-primary shadow-sm float-end mx-1" @click="addNew">
        <i class="bi bi-plus  text-white-50"></i> {{ $t('New') }}
    </button>
</div>
...
<div v-else> 
    ...
    <table class="table table-striped table-sm table-centered mb-2 border bg-white">
        ...
        <tbody>
            <tr v-for="user in usersList" v-bind:key="user.id">
                ...
                <td><i class="bi bi-check text-success" v-if="user.email_verified"></i><i class="bi bi-times text-danger" v-else></i> {{ user.email }}</td>
                <td>{{ user.phone_no }}</td>
                <td>
                ...
                <td class="table-action">
                    ...
                    <router-link :to="{ name: 'customer_details', params: { userid: user.id }}" class="btn btn-sm btn-secondary btn-icon-split">
    ...
    <empty-state v-else text="No customers yet" />
</div>
...
export default {
    ...
    mounted () {
        this.isLoading = true
        axios
        .post(this.$myaccount_url+'customers/list')
        .then(response => {
    ...
    methods: {
        ...
        doDeleteUser: function(){
            ...
            axios
            .post(this.$myaccount_url+'customers/user/delete', {
                _method: 'delete',
