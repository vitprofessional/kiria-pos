...
class CustomerController extends Controller
{
    ...
    public function createinstallment(Request $request){
        ...
        $has_reviewed = $this->transactionUtil->hasReviewed($request->installmentdate);
        ...
        $reviewed = $this->transactionUtil->get_review($request->installmentdate,$request->installmentdate);
            
        if(!empty($reviewed)){
            $output = [
                'success' => 0,
                'msg'     =>"You can't add an installment for an already reviewed date",
            ];
  ...
  public function  createinstallment2(Request $request){
    ...
    public function destroy($id)
    {
        ...
        $reviewed = $this->transactionUtil->get_review($data->installmentdate,$data->installmentdate);
            
        if(!empty($reviewed)){
            $output = [
                'success' => 0,
                'msg'     =>"You can't delete an installment for an already reviewed date",
            ];
...
public function contactwithinstallment(Request $request){
    ...
    $installments=Contact::select('contacts.name','contacts.id as id','installments_table.payment_id as paid_date', DB::raw('count(*) as count'))
        ...
        $query->where('contacts.created_by', '=', session('user.id'));
            if(!$request->installment_id) {
                if ($request->datefrom && $request->dateto)
                 ...
                 ->join('installments','contacts.id','=','installments_table.contact_id')
                ->groupBy('contacts.id')
                ->get();
