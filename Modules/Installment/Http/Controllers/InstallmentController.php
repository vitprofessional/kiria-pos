...
class InstallmentController extends Controller
{
    ...
    public function instalments(Request $request){
        ...
        $installments =Installments::where('installments_table.business_id', $business_id)
        ->join('transactions as t','t.id','installments_table.transaction_id')
              ->where(function ($query) use($request){
                  if($request->id>0)
                      $query->where('installments_table.contact_id','=',$request->id);
            ...
            ->join('contacts','installments_table.contact_id','=','contacts.id')
            ->select(['payment_id','installment_number','contacts.name','installmentdate','installment_value','benefit_value',DB::raw('installment_value+benefit_value as total_value'), 'paid_date', 'installments_table.id'])
...
public function installments_for_home(Request $request){
        ...
        ->join('transactions as t','t.id','installments_table.transaction_id')
            ...
            ->select(['payment_id','installment_number','contacts.name','cg.name as cg_name','installmentdate','installment_value','benefit_value',DB::raw('installment_value+benefit_value as total_value'), 'installments_table.id']);
            //->orderby('installmentdate');
