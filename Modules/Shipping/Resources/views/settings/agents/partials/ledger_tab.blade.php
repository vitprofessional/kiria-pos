    ...
    <table class="table table-bordered table-striped" id="rp_log_table">
        ...
        <tbody>
            ...
            @endphp
                ...
                @php
                    if(in_array($one->type,['commission','agent_payment']) && !empty($one->parent_id)){
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @elseif($one->type == 'agent_payment')
                            {{__('shipping::lang.shipping_agent_payment')}}
                        @elseif($one->type == 'shipping_agent_ob')
                            {{__('shipping::lang.opening_balance')}}
                        @endif
                    </td>
                    <td>
                        @if($one->type == 'agent_payment' || ($one->type == 'shipping_agent_ob' && $one->amount < 0))
                            {{@num_format(abs($one->amount))}}
                        @endif
                    </td>
                    <td>
                        @if($one->type == 'commission'  || ($one->type == 'shipping_agent_ob' && $one->amount > 0))
                            {{@num_format(abs($one->amount))}}
                        @endif
                    </td>
                    <td>
