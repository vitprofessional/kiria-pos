...
class ModeController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $types = ShippingMode::leftjoin('users', 'shipping_mode.created_by', 'users.id')
                ->where('shipping_mode.business_id', $business_id)
                ->select(['shipping_mode.*', 'users.username as created_by']);

            if (!empty(request()->shipping_mode)) {
                $types->where('shipping_mode', request()->shipping_mode);
            }
            ...
            return DataTables::of($types)
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\ModeController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</a></li>';
                    }
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\ModeController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                    }
    ...
    public function create()
    {
        ...
        return view('shipping::settings.mode.create')->with(compact('business_id'));
    }
    ...
    {
        $type = ShippingMode::find($id);

        return view('shipping::settings.mode.edit')->with(compact('type'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            ShippingMode::where('id', $id)->update($data);
