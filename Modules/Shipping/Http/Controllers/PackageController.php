...
class PackageController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $types = ShippingPackage::leftjoin('users', 'shipping_packages.created_by', 'users.id')
                ->where('shipping_packages.business_id', $business_id)
                ->select(['shipping_packages.*', 'users.username as created_by']);

            if (!empty(request()->package_name)) {
                $types->where('package_name', request()->package_name);
            }
            ...
            return DataTables::of($types)
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\PackageController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</a></li>';
                    }
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\PackageController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                    }
    ...
    public function create()
    {
        ...
        return view('shipping::settings.package.create')->with(compact('business_id','shipping_modes'));
    }
    ...
    public function edit($id)
    {
        ...
        $type = ShippingPackage::find($id);
        
        $shipping_modes = ShippingMode::where('shipping_mode.business_id', $business_id)
            ->select('shipping_mode')->get()->pluck('shipping_mode','id');

        return view('shipping::settings.package.edit')->with(compact('type','shipping_modes'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            ShippingPackage::where('id', $id)->update($data);
    ...
    {
        try {
            ShippingPackage::where('id', $id)->delete();
