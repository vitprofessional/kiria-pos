...
class DeliveryController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $types = ShippingDelivery::leftjoin('users', 'shipping_delivery.created_by', 'users.id')
                ->where('shipping_delivery.business_id', $business_id)
                ->select(['shipping_delivery.*', 'users.username as created_by']);

            if (!empty(request()->shipping_delivery)) {
                $types->where('shipping_delivery', request()->shipping_delivery);
            }
            ...
            return DataTables::of($types)
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\DeliveryController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</a></li>';
                    }
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\DeliveryController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                    }
    ...
    public function create()
    {
        ...
        return view('shipping::settings.delivery.create')->with(compact('business_id','shipping_modes'));
    }
    ...
    public function edit($id)
    {
        ...
        $type = ShippingDelivery::find($id);
        
        $shipping_modes = ShippingMode::where('shipping_mode.business_id', $business_id)
            ->select('shipping_mode')->get()->pluck('shipping_mode','id');

        return view('shipping::settings.delivery.edit')->with(compact('type','shipping_modes'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            ShippingDelivery::where('id', $id)->update($data);
    ...
    {
        try {
            ShippingDelivery::where('id', $id)->delete();
