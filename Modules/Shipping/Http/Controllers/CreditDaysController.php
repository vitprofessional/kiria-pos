...
class CreditDaysController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $types = ShippingCreditDay::leftjoin('users', 'shipping_credit_days.created_by', 'users.id')
                ->where('shipping_credit_days.business_id', $business_id)
                ->select(['shipping_credit_days.*', 'users.username as created_by']);

            if (!empty(request()->credit_days)) {
                $types->where('credit_days', request()->credit_days);
            }
            ...
            return DataTables::of($types)
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\CreditDaysController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</a></li>';
                    }
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\CreditDaysController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                    }
    ...
    public function create()
    {
        ...
        $shipping_modes = ShippingMode::where('shipping_mode.business_id', $business_id)
            ->select('shipping_mode')->get()->pluck('shipping_mode','id');
            
        return view('shipping::settings.credit_days.create')->with(compact('business_id','shipping_modes'));
    }
    ...
    public function edit($id)
    {
        ...
        $type = ShippingCreditDay::find($id);
        
        $shipping_modes = ShippingMode::where('shipping_mode.business_id', $business_id)
            ->select('shipping_mode')->get()->pluck('shipping_mode','id');

        return view('shipping::settings.credit_days.edit')->with(compact('type','shipping_modes'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            ShippingCreditDay::where('id', $id)->update($data);
    ...
    {
        try {
            ShippingCreditDay::where('id', $id)->delete();
