...
class StatusController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            $types = ShippingStatus::leftjoin('users', 'shipping_status.created_by', 'users.id')
                ->where('shipping_status.business_id', $business_id)
                ->select(['shipping_status.*', 'users.username as created_by']);

            if (!empty(request()->shipping_status)) {
                $types->where('shipping_status', request()->shipping_status);
            }
            ...
            return DataTables::of($types)
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\StatusController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</a></li>';
                    }
                    ...
                    if (
                        ...
                        $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\StatusController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</a></li>';
                    }
    ...
    public function create()
    {
        ...
        return view('shipping::settings.status.create')->with(compact('business_id'));
    }
    ...
    {
        $type = ShippingStatus::find($id);

        return view('shipping::settings.status.edit')->with(compact('type'));
    }
    ...
    {
        try {
            ShippingStatus::where('id', $id)->delete();
