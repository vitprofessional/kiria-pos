...
class DriverController extends Controller
{
                ...
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if (auth()->user()->can('fleet.drivers.edit')) {
                            $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\DriverController@edit', [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        }
                        if (auth()->user()->can('fleet.drivers.edit')) {
                            $html .= '<li><a href="#" data-href="' . action('\Modules\Shipping\Http\Controllers\DriverController@destroy', [$row->id]) . '" class="delete_button"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</a></li>';
                        }
                        $html .= '<li class="divider"></li>';
                        $html .= '<li><a href="' . action('\Modules\Shipping\Http\Controllers\DriverController@show', [$row->id]) . '?tab=ledger" class=""><i class="fa fa-anchor"></i> ' . __("lang_v1.ledger") . '</a></li>';
    ...
    public function create()
    {
        ...
        return view('shipping::settings.drivers.create')->with(compact(
    ...
    public function show($id)
    {
        ...
        return view('shipping::settings.drivers.show')->with(compact(
            'driver_dropdown',
            'view_type',
            'driver',
            'contact_id'
    ...
    public function edit($id)
    {
        ...
        $business_id = request()->session()->get('business.id');
        $enable_hrm = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'hr_module');
        return view('shipping::settings.drivers.edit')->with(compact(
            'driver', 'enable_hrm'
        ));
    }
