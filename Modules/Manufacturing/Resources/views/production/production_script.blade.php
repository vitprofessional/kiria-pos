<script type="text/javascript">
	...
	$(document).on('change', '#production_form #variation_id, #production_form #location_id', function () {
		...
		if(variation_id) {
		    $.ajax({
		       url: '/manufacturing/get-lot-numbers?variation_id=' + variation_id,
		       datatType: 'json',
		       success: function(ress) {
		           $('#lot_number').val(ress.lot_numbers);
		           $('#inp-RGN_LOT_PREFIX').html(ress.lot_prefix);
		       }
		    });
		}
    ...
    // Add by rmtemplate
    function calculateLotNo() {
        var sumLotNo = 0;
        $(".lot_no").each(function() { 
            var str = this.value.trim();  // .trim() may need a shim
            if (str) {   // don't send blank values to `parseInt`
                sumLotNo += parseInt(str, 10);
            }
        })
        
        var lotPrefix = $('.lot_prefix').val();
        
        
        
        $('#inp-RGN_LOT_PREFIX').html(lotPrefix);
        
        $('#lot_number').val(sumLotNo);
