...
class MemberGroupController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $data = $request->except('_token');
             $exists = MemberGroup::where('member_group', $data['member_group'])
          ...
          if (!$exists) {
            ...
            $output = [
               ...
               $output = [
                'success' => false,
                'tab' => 'member_group',
               'msg' => __('member::lang.member_group_already_exists') 
            ];
            }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data['date'] = !empty($data['date']) ? \Carbon::parse($data['date'])->format('Y-m-d') : date('Y-m-d');
        // Check if another member group with the same name exists, excluding the current one being updated
        $exists = MemberGroup::where('member_group', $data['member_group'])
        ...
        if (!$exists) {
            ...
            // Prepare output for existing member group
            $output = [
                'success' => false,
                'tab' => 'member_group',
                'msg' => __('member::lang.member_group_already_exists') // Create this translation in your lang file
            ];
        }
        } catch (\Exception $e) {
            Log::emergency('File: ' . $e->getFile() . 'Line: ' . $e->getLine() . 'Message: ' . $e->getMessage());
