...
class ServiceAreasController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $data = $request->except('_token');
             ...
             $exists = ServiceArea::where('service_area', $data['service_area'])
                  ->exists();
          ...
          if (!$exists) {
            ...
            $output = [
               ...
               $output = [
                'success' => false,
                'tab' => 'service_areas',
                'msg' => __('member::lang.service_area_already_exists')
            ];
            }
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Check if another service area with the same name exists, excluding the current one
        $exists = ServiceArea::where('service_area', $data['service_area'])
            ->where('id', '!=', $id) // Exclude the current record by its ID
            ->exists();
        ...
        if (!$exists) {
            ...
            // If duplicate service area exists, return an error message
            $output = [
                'success' => false,
                'tab' => 'service_areas',
                'msg' => __('member::lang.service_area_already_exists') // Make sure this translation exists
            ];
        }
        } catch (\Exception $e) {
            Log::emergency('File: ' . $e->getFile() . 'Line: ' . $e->getLine() . 'Message: ' . $e->getMessage());
