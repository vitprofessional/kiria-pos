	            ...
	            $.ajax({
	                ...
	                success: function(result) {
	                    if (result.success) {
	                        ...
	                        var lead_view = 'list_view'; //urlSearchParam('lead_view');
...
// 	};
	...
	function updateLeadLifeStageForKanban(data, el) {
		$.ajax({
	        method: 'GET',
	        dataType: 'json',
	        url: '/crm-module/lead/' + data.lead_id + '/post-life-stage',
	        data:data,
	        success: function(result){
	            if (result.success) {
...
function get_todays_schedule() {
	$.ajax({
        method: 'GET',
        dataType: "json",
        url: '/crm-module/todays-follow-ups',
        success: function(result){
            if (result.success) {
...
function initializeLeadScheduleDatatable() {
	if((typeof lead_schedule_datatable == 'undefined')) {
		lead_schedule_datatable = $("#lead_schedule_table").DataTable({
		        ...
		        serverSide: true,
		        ajax: {
		            url: "/crm-module/lead-follow-ups",
		            data:function(d) {
...
function initializeLeadDatatable() {
	if((typeof leads_datatable == 'undefined')) {
		...
		leads_datatable = $("#leads_table").DataTable({
		        ...
		        scrollCollapse: true,
		        ajax: {
		            url: "/crm-module/leads",
		            data:function(d) {
		            	d.source = $("#source").val();
		            	d.life_stage = $("#life_stage").val();
		            	d.user_id = $("#user_id").val();
		            	d.lead_view = 'list_view'; //urlSearchParam('lead_view');
		            }
		        },
		        columnDefs: [
		            {
