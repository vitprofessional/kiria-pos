...
class HmsController extends Controller
{
    ...
    public function index()
    {
      ...
      // Assuming __chartOptions() expects a title (from translation)
        $booking_chart->labels($labels)
            ->options(
                $this->__chartOptions(
                    __('hms::lang.bookings') // Correctly formatted translation
                )
            );
      ...
      $past_booking_chart->labels($labels)
            ->options(
                $this->__chartOptions(
                    __('hms::lang.bookings') // Removed trailing comma
                )
            );
        ...
        return view('hms::dashboard', compact('booking_chart','past_booking_chart', 'room_count', 'unbooked_rooms_by_type', 'guest_count_tonight', 'arrive_today', 'leave_today', 'today_arrivales', 'today_departure', 'latest_bookig'));
    }
