...

{
...

    {
        ...
        if (request()->ajax()) {
            ...
            $payrolls = $this->essentialsUtil->getPayrollQuery($business_id)->distinct();;
            ...
            if (! empty(request()->input('location_id'))) {
                ...
                $payrolls->where('epg.location_id', request()->input('location_id'));
        ...
        if (auth()->user()->can('essentials.create_payroll')) {
            ...
            $employees = $this->__getHRMEmployeesByLocation($business_id);
...

    {
        ...
        try {
            ...
            // if (! ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.create_payroll')) {
            ...
            if (! empty($add_payroll_for)) {
                ...
                foreach ($employees as $employee) {
                    ...
                    $payrolls[$employee->id]['name'] = $employee->name;
...

    {
        ...
        $final_total_in_words = $this->commonUtil->convertAmountToWords($payroll->final_total);
...

    {
        ...
        // if (! (! $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.update_payroll')) {
...

    {
        ...
        // if (! ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.delete_payroll')) {
...

    {
        ...
        // if (! ($is_admin || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
...

    {
        ...
        // if (! (auth()->user()->can('essentials.update_payroll') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
...

    {
        ...
        try {
            ...
            $employees = $this->__getHRMEmployeesByLocation($business_id, $location_id);
    ...
    private function __getHRMEmployeesByLocation($business_id, $location_id = null) {
        $query = EssentialsEmployee::where('business_id', $business_id);
        $employee = $query->select('id', 'name')->get();
