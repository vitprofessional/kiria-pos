...
class ToDoController extends Controller
{
    ...
    public function create()
    {
        ...
        if (! ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.add_todos')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        if (! ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.edit_todos')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        if (! ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) && ! auth()->user()->can('essentials.delete_todos')) {
            abort(403, 'Unauthorized action.');
        }
