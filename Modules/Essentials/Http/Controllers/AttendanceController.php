...
class AttendanceController extends Controller
{
    ...
    public function clockInClockOut(Request $request)
    {
        ...
        if(! $request->has('image') || is_null($request->image)) {
            return ['success' => false,
                'msg' => __('essentials::lang.photo_required'),
            ];
        }

        $response = Http::asForm()->post(env('FACE_PLUS_URL'), [
            'api_key' => env('FACE_PLUS_API_KEY'),
            'api_secret' => env('FACE_PLUS_API_SECRET'),
            'image_base64_1' => base64_encode(File::get(public_path('uploads/user-profile-photos/'.auth()->user()->profile_photo))),
            'image_base64_2' => $request->image
        ]);

        if(! $response->ok() || is_null($response->json('confidence')) || $response->json('confidence') < 80) {
            return ['success' => false,
                'msg' => __('essentials::lang.photo_does_not_match'),
            ];
        }
        ...
        try {
            ...
            if ($request->hasFile('attendance')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    //Add Employee
                    if (! empty($value[0])) {
                        $temp['employee_name'] = trim($value[0]);
                    } else {
                        $is_valid = false;
                        $error_msg = "Employee's name is required in row no. $row_no";
                        break;
                    }
                    if (! empty($value[1])) {
                        $temp['empoyee_code'] = trim($value[1]);
                    } else {
                        $is_valid = false;
                        $error_msg = "Employee code is required in row no. $row_no";
                        break;
                    }
                    //Add user
                    if (! empty($value[2])) {
                    ...
                    //clockin time
                    if (! empty($value[3])) {
                        $temp['clock_in_time'] = trim($value[3]);
                    } else {
                        $is_valid = false;
                        $error_msg = "Clock in time is required in row no. $row_no";
                        break;
                    }
                    $temp['clock_out_time'] = ! empty($value[4]) ? trim($value[4]) : null;

                    //Add shift
                    if (! empty($value[5])) {
                        $shift_name = trim($value[5]);
                    ...
                    $temp['clock_in_note'] = ! empty($value[6]) ? trim($value[6]) : null;
                    $temp['clock_out_note'] = ! empty($value[7]) ? trim($value[7]) : null;
                    $temp['ip_address'] = ! empty($value[8]) ? trim($value[8]) : $ip_address;
