...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif" hidden>
    ...
    <table class="table text-left align-left no-border @if(!empty($for_pdf)) table-pdf @endif" id="customer_detail_table">
        <tbody>
            ...
            <tr>
                ...
                <td>{{@num_format($ledger_details['balance_details']['total_purchase'])}}
                </td>
            </tr>
            @endif
            ...
            <tr>
                ...
                <td>{{@num_format($ledger_details['balance_details']['total_sale'])}}
                </td>
            </tr>
            @endif
            <tr>
                <td>@lang('sale.total_paid')</td>
                <td>{{@num_format($ledger_details['balance_details']['total_paid'])}}
                </td>
            </tr>
            <tr>
                <td><strong>@lang('lang_v1.balance_due')</strong></td>
                <td>{{@num_format($ledger_details['balance_details']['total_balance'])}}</td>
            </tr>
        </tbody>
    </table>
</div>
...
<input type="hidden" id="balance_due" value="{{$ledger_details['balance_details']['total_balance']}}">