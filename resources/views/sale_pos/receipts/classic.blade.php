...
<div class="row" style="font-size: {{$h_font_size}}px !important;">
	...
	<div class="col-xs-12 text-{{$header_align}}">
		...
		<p>
			...
			{{ str_replace("Quotations", 'Quotation', $receipt_details->sub_heading_line1) }}
			@endif
			@if(!empty($receipt_details->sub_heading_line2))
			<br>{{ str_replace("Quotations", 'Quotation', $receipt_details->sub_heading_line2) }}
			@endif
			@if(!empty($receipt_details->sub_heading_line3))
			<br>{{ str_replace("Quotations", 'Quotation', $receipt_details->sub_heading_line3) }}
			@endif
			@if(!empty($receipt_details->sub_heading_line4))
			<br>{{ str_replace("Quotations", 'Quotation', $receipt_details->sub_heading_line4) }}
			@endif
			@if(!empty($receipt_details->sub_heading_line5))
			<br>{{ str_replace("Quotations", 'Quotation', $receipt_details->sub_heading_line5) }}
			@endif
		</p>
		<p>
		...
		<p>
			@if(!empty($receipt_details->quotation_no))
				<b>@lang('lang_v1.quotation_no'):</b> {{ $receipt_details->quotation_no }}
			@endif
		</p>
		...
		<table class="table table-responsive">
			...
			<tbody>
				...
				<tr>
					...
					<!-- <td style="font-size: {{$font_size}}px !important;">{{$line['unit_price_before_discount']}}</td> -->
					<td style="font-size: {{$font_size}}px !important;">{{$line['default_sale_price']}}</td>
				...
				<tr>
					...
					<td>{{$modifier['unit_price_before_discount']}}</td>
...
<div class="row" style="font-size: {{$f_font_size}}px !important;">
	...
	<div class="col-xs-6">
		...
		<table class="table table-condensed">
			...
			@if(!empty($receipt_details->paid_greater) &&$receipt_details->paid_greater === true)
				<tr>
					<th>
						Balance Return
					</th>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							{{$receipt_details->subtotal_final}}
						</td>
					</tr>
...
<div class="row">
	<div class="col-xs-12">
		@if(!empty($receipt_details->footer_text))
			...
			@else
				{!! str_replace("invoice", 'quotation', $receipt_details->footer_text) !!}
			@endif
		@endif
	</div>
</div>
