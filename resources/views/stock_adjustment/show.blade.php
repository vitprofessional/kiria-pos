      					...
      					<table class="table table-condensed bg-gray">
							...
							@foreach( $stock_adjustment->stock_adjustment_lines as $stock_adjustment_line )
								<tr>
						            ...
						            @endif
									<td>
										{{@format_quantity($stock_adjustment_line->quantity * (($stock_adjustment->stock_adjustment_type == 'decrease') ? -1 : 1))}}
									</td>
									<td>
										{{@num_format($stock_adjustment_line->unit_price)}}
									</td>
									<td>
										{{@num_format($stock_adjustment_line->unit_price * $stock_adjustment_line->quantity * (($stock_adjustment->stock_adjustment_type == 'decrease') ? -1 : 1))}}
									</td>
								</tr>
							@endforeach
						</table>
				    ...
				    <div class="table-responsive">
				        <table class="table no-border">
				          	<tr>
				            	...
				            	<td><span class="display_currency pull-right" data-currency_symbol="true">{{ $stock_adjustment->final_total * (($stock_adjustment->stock_adjustment_type == 'decrease') ? -1 : 1) }}</span></td>
				          	</tr>
				          	<tr>
