<div class="modal-dialog" role="document">
  <div class="modal-content">
    ...
    {!! Form::open(['url' => url('taxonomies'), 'method' => 'post', 'id' => 'category_add_form' ]) !!}
    ...
    <div class="modal-body">
      ...
      @php
        ...
        if($category_type == "hrm_designation"){
            ...
            $parent_categories = $departments;
            $label = __( 'category.select_department' ) . ':';
            $status = true;
        }else{
            $label = __( 'category.select_parent_category' ) . ':';
            $status = false;
        }
      ...
      @if(!empty($parent_categories) && $enable_sub_category)
        <div class="form-group" @if($category_type == "hrm_designation") hidden @endif>
            <div class="checkbox">
              <label>
                 {!! Form::checkbox('add_as_sub_cat', 1, $status,[ 'class' => 'toggler', 'data-toggle_id' => 'parent_cat_div' ]); !!} @lang( 'lang_v1.add_as_sub_txonomy' )
              </label>
            </div>
        </div>
        <div class="form-group @if($category_type != "hrm_designation") hide @endif " id="parent_cat_div">
          {!! Form::label('parent_id', $label) !!}
