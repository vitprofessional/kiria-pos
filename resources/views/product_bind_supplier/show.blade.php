...
function get_contact_payment() {
    ...
    $.ajax({
        url: '/contacts/payments?contact_id={{$contact->id}}&start_date=' + start_date + '&transaction_type=' + transaction_type + '&show_payments=' + show_payments + '&end_date=' + end_date+'&transaction_amount='+transaction_amount,
        dataType: 'html',
        success: function(result) {
            $('#contact_payment_div')
                .html(result);
...
$(document).on('click', '#send_ledger', function() {
    ...
    var url = "{{action('NotificationController@getTemplate', [$contact->id, 'send_ledger'])}}" + '?start_date=' + start_date + '&end_date=' + end_date + '&contact_type=' + contact_type;
    $.ajax({
        url: url,
        dataType: 'html',
        success: function(result) {
...
<script>
    ...
    $(document).on('click', '#add_discount', function(e) {
        ...
        //$("#account_type").find("option:selected").prop('selected',false);
        $('#account_sub_type option,#sub_type option').remove(); // clear all values 
        $('#account_sub_type').append('<option value="" selected>Please Select</option>');
        $('#add_discount_modal').modal('show');
        $(document).find("#account_type").val('').trigger('change');
    });
     //Date picker
