...
@section('javascript')
    ...
    $('#form_date_range').on('apply.daterangepicker', function(ev, picker) {
        if (picker.chosenLabel === 'Custom Date Range') {
            $('#target_custom_date_input').val('form_date_range');
            $('.custom_date_typing_modal').modal('show');
        }
    });
    $('#custom_date_apply_button').on('click', function() {
        if($('#target_custom_date_input').val() == "form_date_range"){
            ...
            if (startDate.length === 10 && endDate.length === 10) {
                ...
                $('#form_date_range').data('daterangepicker').setStartDate(moment(startDate));
                $('#form_date_range').data('daterangepicker').setEndDate(moment(endDate));
