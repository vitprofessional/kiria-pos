    ...
    $('#stock_summary_filter_form #summary_location_id').change(function() {
		...
		$.ajax({
			...
			success: function(result) {
                ...
                $('#summary_store_id').append(`<option value= "">`+option_value+`</option>`);
				$.each(result, function(i, location) {
					$('#summary_store_id').append(`<option value= "`+location.id+`">`+location.name+`</option>`);
                });
...
$(document).ready(function(){
    ...
    function stocksummaryUpdate(){
        ...
        let end = null;
         if($('#stock_summary_date_range').length){
            var stock_start = $('input#stock_summary_date_range')
                .data('daterangepicker')
                .startDate.format('YYYY-MM-DD');
            var stock_end = $('input#stock_summary_date_range')
                .data('daterangepicker')
                .endDate.format('YYYY-MM-DD');
        ...
        $.ajax({
            ...
            success: function(data) {
                 ...
                 $('#stock_summary').find('.purchase_amount').html(__currency_trans_from_en(data.purchase_amount));
                 $('#stock_summary').find('.opening_amount').html(__currency_trans_from_en(data.opening_amount));
                 $('#stock_summary').find('.balance_amount').html(__currency_trans_from_en(data.balance_amount));
            },
        });
    }
    ...
    $('.category_id, .sub_category_id').change(function(){
        ...
        var summary_cat = $('#summary_product_list_filter_category_id').val();
        ...
        $.ajax({
            ...
            url: '/products/get_sub_categories',
            dataType: 'html',
            data: { cat_id: summary_cat },
            success: function(result) {
                if (result) {
                    $('#summary_product_list_filter_sub_category_id').html(result);
                }
            },
        });
        
        
        var summary_sub_cat = $('#summary_product_list_filter_sub_category_id').val();
        ...
        $.ajax({
            ...
            url: '/products/get_product_category_wise',
            dataType: 'html',
            data: { cat_id: summary_cat , sub_cat_id: summary_sub_cat },
            success: function(result) {
                if (result) {
                    $('#summary_product_list_filter_product_id').html(result);
                }
            },
        });
    });
...
<script>
    ...
    $('#product_transaction_location_id'
        ...
        option_value = "{{__('lang_v1.all')}}";
        if(!$(this).val()){
            $('#product_transaction_store_id').empty();
            $('#product_transaction_store_id').append(`<option value= "">`+option_value+`</option>`);
            return;
        }
		$.ajax({
			method: 'get',
			...
			success: function(result) {
                ...
                $('#product_transaction_store_id').append(`<option value= "">`+option_value+`</option>`);
				$.each(result, function(i, location) {
					$('#product_transaction_store_id').append(`<option value= "`+location.id+`">`+location.name+`</option>`);
                });
                $("#product_transaction_store_id").change();
			},
        });
    ...
    var product_transaction_report_cols = [
            ...
            { data: 'description', name: 'invoice_no' },
            ...
            { data: 'balance_qty_value_date_wise', name: 'balance_qty_value_date_wise',searchable: false },
            { data: 'date', name: 'p.date' }
